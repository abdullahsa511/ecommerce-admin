...
let handle = document.querySelector('.dd-handle');
...
function isValidDropTarget(element) {
    ...
    if (!element.hasClass(options.itemClass)) return false;
    
    // Check depth limit
    const depth = element.parents(options.listNodeName).length;
    if (depth > options.maxDepth) return false;
    
    // Check if groups match
    if (options.group !== element.closest('.' + options.rootClass).data('nestable-group')) {
        return false;
    }
...
function handleReordering(element, direction) {
    const parent = element.parent();
    const list = parent.find(options.listNodeName).last();
    
    // Handle nested lists
    if (direction === 'down' && !list.length) {
        list = $('<' + options.listNodeName + '/>').addClass(options.listClass);
        list.append(this.placeEl);
        parent.append(list);
    }
    
    // Update hierarchy
    updateHierarchy(element, list);
}
