...
import { IItemProductEdit } from '@/models/Pinboard';
import { PhotoService } from './PhotoService.ts';
import { ref, onMounted } from 'vue';

const emit = defineEmits(['updatePinboardProductItem', 'remove']);

const props = defineProps<{
    item: IItemProductEdit;
}>();

const updateItem = () => {
    emit('updatePinboardProductItem', props.item);
};
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="th-pinboard-item-img">
                    <img :src="selectedImage" class="img-fluid rounded-start" alt="Product Image" style="cursor:pointer;" @click="openModal(images.findIndex(img => img.itemImageSrc === selectedImage))">
                </div>
            </div>
            <div class="col-md-8">
                <div class="th-pinboard-item-info">
                    <div class="card-body ">
                        ...
                        <div class="th-pinboard-info-body">
                            <h4 class="fw-bold font-wight-700">Product</h4>
                            <h4 class="card-title font-wight-700">
                                {{ item.product_name }}
                            </h4>

                            <div v-if="item.product_price !== undefined" class="mb-2">
    ...
    <template v-if="showModal">
      ...
      >
        <div style="position:relative; max-width:90vw; max-height:90vh; display:flex; align-items:center;">
          ...
          <!-- <button @click="closeModal" style="position:absolute; top:-40px; right:0; font-size:2rem; background:none; border:none; color:white; cursor:pointer;">&times;</button> -->
        </div>
      </div>
    </template>
</template>
