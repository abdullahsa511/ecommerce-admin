...
import { IItemBlogPostEdit } from '@/models/Pinboard';
import { PhotoService } from './PhotoService.js';
import { ref, onMounted } from 'vue';

const emit = defineEmits(['updatePinboardBlogPostItem', 'remove']);

const props = defineProps<{
    item: IItemBlogPostEdit;
}>();

const updateItem = () => {
    emit('updatePinboardBlogPostItem', props.item);
};
...
<template>
    <div class="th-pinboard-item position-relative">
        <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-2" aria-label="Remove" @click="$emit('remove', item)">
            <i class="fa fa-times">x</i>
        </button>
        <div class="row">
            <div class="col-md-4">
                <div class="th-pinboard-item-img">
                    <img :src="selectedImage" class="img-fluid rounded-start" alt="Blog Post Image" style="cursor:pointer;" @click="openModal(images.findIndex((img: any) => img.itemImageSrc === selectedImage))">
                </div>
            </div>
            <div class="col-md-8">
                <div class="th-pinboard-item-info">
                    <div class="card-body position-relative">
                        <button type="button" class="btn-close position-absolute top-0 end-0 p-2" aria-label="Close" @click="updateItem"></button>
                        <div class="th-pinboard-info-body">
                            <h4 class="fw-bold font-wight-700">Blog Post</h4>
                            ...
                            <div v-if="item.post_description" class="mb-2">
                                <strong>Description:</strong> {{ item.post_description }}
                            </div>
                            <div v-if="item.post_content" class="mb-2">
                                <strong>Content:</strong> {{ item.post_content }}
                            </div>
                            <!-- Comment Box -->
                            ...
                            <div class="th-doc-actions th-pinbord-item-comment">
                                <a href="#" class="th-btn-gray text-capitalize">
                                    <span class="mr-5">Update Blog Post</span>
                                </a>
                            </div>
                        </div>
                    </div>
