...
onMounted( async() => {
    try {
        ...
        const fetchedTaxType = await taxTypeStore.fetchTaxTypeById(taxTypeId);
        newTaxType.value = new TaxType(fetchedTaxType);
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
...
const updateTaxType = async () => {
    try {
        await taxTypeStore.updateTaxType(taxTypeId, newTaxType.value);
        toast.add({
            severity: 'success',
            summary: 'Success',
            detail: 'Tax type updated successfully',
            life: 3000
        });
        goToTaxTypeList();
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to update tax type. Please try again.',
            life: 3000
        });
    }
};
