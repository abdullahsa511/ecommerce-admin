...
onMounted(async () => {
    ...
    if (tagId) {
        try {
            const tag = await postTagStore.fetchPostTagById(tagId);
            if (tag) {
                newTag.value = new PostTag(tag);
                
                // Set up image data for the upload component
                if (newTag.value.image && newTag.value.image !== '') {
                    newTag.value.uploadedImages = [{
                        name: newTag.value.image.split('/').pop(),
                        image: newTag.value.image,
                        objectURL: newTag.value.image.startsWith('http') ? newTag.value.image : serverUrl + newTag.value.image,
                        status: { name: 'Uploaded', severity: 'success' }
                    }];
                    console.log('Image data set:', newTag.value.uploadedImages);
                } else {
                    newTag.value.uploadedImages = [];
                }
            }
            ...
            toast.add({
                severity: 'error',
                summary: 'Error',
                detail: 'Failed to load post tag data',
                life: 3000
            });
        }
    }
...
const saveTag = async () => {
    try {
        const tagId = route.params.id as string;
        await postTagStore.updatePostTag(tagId, newTag.value);
        toast.add({
            severity: 'success',
            summary: 'Success',
            detail: 'Post tag updated successfully',
            life: 3000
        });
        goToPostTagList();
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to update post tag. Please try again.',
            life: 3000
        });
    }
};
