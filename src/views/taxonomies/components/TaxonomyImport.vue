<script setup lang="ts">
  ...
  const { importExport, fb } = storeToRefs(taxonomyStore);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await taxonomyStore.taxonomyImport(formData);
      toast.add({
        severity: 'success',
        summary: 'Success',
  ...
  // Clear results
  const clearResults = () => {
    importExport.value = new TaxonomyImportResponse();
  };
  // Clear filters
  const clearFilter = () => {
  ...
  <template>
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">Import Taxonomy CSV data</h2>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div v-if="selectedFile" class="flex flex-col gap-4">
          ...
          <div v-if="fb.loading['taxonomy.import']" class="w-full">
          ...
          <Button icon="pi pi-save" :label="fb.loading['taxonomy.import'] ? 'Importing...' : 'Import '" severity="success"
            @click="handleSave" :loading="fb.loading['taxonomy.import']" :disabled="fb.loading['taxonomy.import']"
            class="p-button-md" />
        </div>
      </div>
    </div>
    ...
    <div v-if="showResults" class="bg-white rounded-lg p-6 shadow-md">
      ...
      <div v-if="importExport.invalid_records > 0 && importExport.invalid_data" class="mb-6">
        ...
        <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="200" dataKey="row"
          filterDisplay="menu" :globalFilterFields="['name', 'post_type', 'type']" class="th-table">
          <template #header>
          ...
          <Column field="type" header="Taxonomy Type" style="min-width: 10rem">
            <template #body="{ data }">{{ data.data?.type || data.type || '' }}</template>
          </Column>
          <Column field="post_type" header="Model Type" style="min-width: 10rem">
            <template #body="{ data }">{{ data.data?.post_type || data.post_type || '' }}</template>
          </Column>
      ...
      <div v-if="importExport.valid_records > 0 && importExport.valid_data">
        ...
        <DataTable v-model:filters="filters" :value="importExport.valid_data" paginator :rows="200" dataKey="attribute_id"
          ...
          <Column field="post_type" header="Model Type" style="min-width: 10rem">
            <template #body="{ data }">{{ data.post_type }}</template>
          </Column>
        </DataTable>
      </div>
    ...
    <div v-if="fb.errors['taxonomy.import']" class="text-red-600 text-sm pt-5">
      <div v-html="fb.errors['taxonomy.import']"></div>
    </div>
  </template>
