...
const { importExport, fb } = storeToRefs(orderStatusStore);
const toast = useToast();

// Local
const selectedFileOrderStatuses = ref<File | null>(null);
const filters = ref({ global: { value: null, matchMode: 'contains' } });
const showOrderStatusesResults = computed(() => {
...
const handleFileSelectOrderStatuses = (event: any) => {
  ...
  selectedFileOrderStatuses.value = file;
};

// Handle attribute import
const handleSaveOrderStatuses = async () => {
  if (!selectedFileOrderStatuses.value) {
    toast.add({
      severity: 'error',
      summary: 'Error',
  ...
  try {
    ...
    formData.append('csv_file', selectedFileOrderStatuses.value);
    await orderStatusStore.orderStatusesImport(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
    ...
    selectedFileOrderStatuses.value = null;
    // emit('saved');
  } catch (error: any) {
...
// Clear results
const clearResults = () => {
  importExport.value = new OrderStatusImportResponse();
};
// Clear filters
const clearFilter = () => {
...
<template>
  <!-- Order Statuses Import Section -->
  <div class="bg-white rounded-lg p-6 shadow-md">
    <h2 class="text-xl font-semibold mb-2">Import Order Statuses CSV data</h2>
    <p class="text-gray-600 mb-4">Upload a CSV file to start importing Order statuses data</p>
    <div class="flex flex-col gap-4">
      <FileUpload
        mode="basic"
        name="importOrderStatusesFile"
        accept=".csv"
        :maxFileSize="1000000"
        @select="handleFileSelectOrderStatuses"
        :auto="false"
        chooseLabel="Select CSV File"
      />
      <div v-if="selectedFileOrderStatuses" class="flex flex-col gap-4">
        <div class="flex items-center gap-2 p-3 bg-gray-50 rounded">
          <i class="pi pi-file text-gray-500"></i>
          <span class="text-gray-700">{{ selectedFileOrderStatuses.name }}</span>
        </div>
        <div v-if="fb.loading['attributeGroup.import']" class="w-full">
          <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
          <p class="text-sm text-gray-600 mt-2 text-center">Importing Order statuses data...</p>
        </div>
        <Button
          icon="pi pi-save"
          :label="fb.loading['orderStatus.import'] ? 'Saving...' : 'Import Order Statuses'"
          severity="success"
          @click="handleSaveOrderStatuses"
          :loading="fb.loading['orderStatus.import']"
          :disabled="fb.loading['orderStatus.import']"
          class="p-button-md"
        />
      </div>
    </div>
  ...
  <div v-if="showOrderStatusesResults" class="bg-white rounded-lg p-6 shadow-md">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-semibold">Order Statuses Import Results</h2>
    ...
    <div v-if="importExport.updated_records > 0 && importExport.updated_data" class="mb-6">
      ...
      >
        ...
        <Column field="order_status_id" header="Id" style="min-width: 10rem">
          <template #body="{ data }">{{ data.order_status_id }}</template>
        </Column>
        <Column field="data.name" header="Name" style="min-width: 10rem">
  ...
  <div v-if="fb.errors['orderStatus.import']" class="text-red-600 text-sm pt-5">
    <div v-html="fb.renderErrors('orderStatus.import')"></div>
  </div>
</template>
