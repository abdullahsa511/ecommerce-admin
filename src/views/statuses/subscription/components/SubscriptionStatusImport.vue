...
const { importExport, fb } = storeToRefs(subscriptionStatusStore);
...
const handleSave = async () => {
  ...
  try {
    ...
    await subscriptionStatusStore.subscriptionStatusesImport(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Subscription statuses imported successfully',
      life: 3000,
    });
...
// Clear results
const clearResults = () => {
  importExport.value = new SubscriptionStatusImportResponse();
};
// Clear filters
const clearFilter = () => {
...
<template>
  <!-- Subscription Statuses Import Section -->
  <div class="bg-white rounded-lg p-6 shadow-md">
    <h2 class="text-xl font-semibold mb-2">Import Subscription Statuses CSV data</h2>
    <p class="text-gray-600 mb-4">Upload a CSV file to start importing Subscription statuses data</p>
    <div class="flex flex-col gap-4">
      <FileUpload
        mode="basic"
        name="importSubscriptionStatusesFile"
        accept=".csv"
        :maxFileSize="1000000"
      ...
      <div v-if="selectedFile" class="flex flex-col gap-4">
        ...
        <div v-if="fb.loading['subscriptionStatus.import']" class="w-full">
          <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
          <p class="text-sm text-gray-600 mt-2 text-center">Importing Subscription statuses data...</p>
        </div>
        <Button
          icon="pi pi-save"
          :label="fb.loading['subscriptionStatus.import'] ? 'Saving...' : 'Import Subscription Statuses'"
          severity="success"
          @click="handleSave"
          :loading="fb.loading['subscriptionStatus.import']"
          :disabled="fb.loading['subscriptionStatus.import']"
          class="p-button-md"
        />
      </div>
    </div>
  ...
  <!-- Subscription Statuses Import Results Section -->
  <div v-if="showResults" class="bg-white rounded-lg p-6 shadow-md">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-semibold">Subscription Statuses Import Results</h2>
      ...
      >
        <template #header>
          <div class="flex justify-between items-center">
            ...
            <InputText v-model="filters['global'].value" placeholder="Search invalid Subscription statuses records..." />
          </div>
        </template>
        <template #empty>No invalid Subscription statuses records found.</template>
        ...
        <Column field="data.language_code" header="Language Code" style="min-width: 10rem">
          <template #body="{ data }">{{ importExport.language_map[data.data.language_id] }}</template>
        </Column>
    ...
    <div v-if="importExport.updated_records > 0 && importExport.updated_data" class="mb-6">
      ...
      >
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search updated Subscription statuses records..." />
          </div>
        </template>
        <template #empty>No updated Subscription statuses records found.</template>
        <Column field="subscription_status_id" header="Id" style="min-width: 10rem">
          <template #body="{ data }">{{ data.subscription_status_id }}</template>
        </Column>
        <Column field="data.name" header="Name" style="min-width: 10rem">
    ...
    <div v-if="importExport.valid_records > 0 && importExport.valid_data">
      ...
      <DataTable
        ...
        filterDisplay="menu"
        :globalFilterFields="['name', 'language_code']"
        class="th-table"
      >
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search valid Subscription statuses records..." />
          </div>
        </template>
        <template #empty>No valid Subscription statuses records found.</template>
  ...
  <div v-if="fb.errors['subscriptionStatus.import']" class="text-red-600 text-sm pt-5">
    <div v-html="fb.renderErrors('subscriptionStatus.import')"></div>
  </div>
</template>

<style scoped>
.product-subscriptionStatus-index {
  padding: 2rem;
}
