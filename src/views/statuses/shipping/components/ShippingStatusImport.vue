...
const { importExport, fb } = storeToRefs(shippingStatusStore);
...
const handleSave = async () => {
  ...
  try {
    ...
    await shippingStatusStore.shippingStatusesImport(formData);
    console.log('importExport after import', importExport.value);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'shipping statuses imported successfully',
      life: 3000,
    });
...
// Clear results
const clearResults = () => {
  importExport.value = new ShippingStatusImportResponse();
};
// Clear filters
const clearFilter = () => {
...
<template>
  ...
  <div class="bg-white rounded-lg p-6 shadow-md">
    <h2 class="text-xl font-semibold mb-2">Import shipping statuses CSV data</h2>
    <p class="text-gray-600 mb-4">Upload a CSV file to start importing shipping statuses data</p>
    <div class="flex flex-col gap-4">
      ...
      <div v-if="selectedFile" class="flex flex-col gap-4">
        ...
        <div v-if="fb.loading['taxType.import']" class="w-full">
          ...
          <p class="text-sm text-gray-600 mt-2 text-center">Importing shipping statuses data...</p>
        </div>
        <Button
          icon="pi pi-save"
          :label="fb.loading['taxType.import'] ? 'Saving...' : 'Import shipping statuses'"
          severity="success"
          @click="handleSave"
  ...
  <div v-if="showResults" class="bg-white rounded-lg p-6 shadow-md">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-semibold">shipping statuses Import Results</h2>
      ...
      >
        <template #header>
          <div class="flex justify-between items-center">
            ...
            <InputText v-model="filters['global'].value" placeholder="Search invalid shipping statuses records..." />
          </div>
        </template>
        <template #empty>No invalid shipping statuses records found.</template>
    ...
    <div v-if="importExport.duplicated_records > 0 && importExport.duplicated_data" class="mb-6">
      ...
      >
        ...
        <template #empty>No duplicated shipping statuses records found.</template>
        <Column field="data.name" header="Name" style="min-width: 10rem">
          <template #body="{ data }">{{ (data.data && data.data.name) ? data.data.name : data.name }}</template>
        </Column>
        <Column field="data.language_code" header="Language Code" style="min-width: 10rem">
    ...
    <div v-if="importExport.valid_records > 0 && importExport.valid_data">
      ...
      >
        ...
        <template #empty>No valid shipping statuses content records found.</template>
    ...
    <div v-if="importExport.updated_records > 0 && importExport.updated_data">
      ...
      >
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search valid shipping statuses records..." />
          </div>
        </template>
  ...
  <div v-if="fb.errors['shippingStatus.import']" class="text-red-600 text-sm pt-5">
    <div v-html="fb.renderErrors('shippingStatus.import')"></div>
  </div>
</template>
