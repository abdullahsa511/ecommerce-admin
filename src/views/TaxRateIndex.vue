...
const isLoading = computed(() => {
  return fb.value.loading?.taxRate === true;
});
...
const deleteSelectedTaxRates = () => {
  if (selectedTaxRates.value && selectedTaxRates.value.length > 0) {
    // Implement delete functionality for selected items
    console.log('Delete selected tax rates:', selectedTaxRates.value);
  }
};

const editTaxRate = (taxRate) => {
  router.push(`/configuration/settings/edit-tax-rates/${taxRate.tax_rate_id}`);
};
...
const addTaxRate = () => {
  router.push('/configuration/settings/add-tax-rates');
};
</script>
...
<template>
  <div>
    ...
    <DataTable
      ...
      v-model:selection="selectedTaxRates"
      :value="taxRates"
      paginator
      :rows="10"
      dataKey="tax_rate_id"
      filterDisplay="menu"
      :loading="isLoading"
      :globalFilterFields="['name', 'rate', 'type']"
    ...
    >
      <template #header>
        <div class="flex justify-between items-center">
          <div class="flex gap-2">
            ...
            />
            <Button
              v-if="selectedTaxRates && selectedTaxRates.length > 0"
              type="button"
              icon="pi pi-trash"
      ...
      <Column header="Rate" filterField="rate" :showFilterMatchModes="false" :filterMenuStyle="{ width: '14rem' }" style="min-width: 12rem">
        <template #body="{ data }">
          {{ data.rate }}%
        </template>
      </Column>

      <Column header="Type" filterField="type" :showFilterMatchModes="false" :filterMenuStyle="{ width: '14rem' }" style="min-width: 12rem">
        <template #body="{ data }">
          {{ data.type }}
        </template>
        <template #filter="{ filterModel }">
