...
<template>
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
        <h2 class="text-xl font-semibold mb-2">Import Product Tags CSV </h2>
        <p class="text-gray-600 mb-4">Upload a CSV file to start importing product tags
            data
        </p>
        <div class="flex flex-col gap-4">
            ...
            <div v-if="selectedFileProductsTags" class="flex flex-col gap-4">
                ...
                <Button icon="pi pi-save" :label="isSavingProductsTags ? 'Saving...' : 'Import Tags'" severity="success"
                    @click="handleSaveProductsTags" :loading="isSavingProductsTags" :disabled="isSavingProductsTags"
                    class="p-button-md" />
            </div>
        </div>
    </div>
    ...
    <div v-if="showProductsTagsResults && productsTagsImportResults" class="bg-white rounded-lg p-6 shadow-md">
        ...
        <div v-if="productsTagsImportResults?.success?.invalid_records > 0 && productsTagsImportResults?.success?.invalid_data"
            ...
            <DataTable v-model:filters="filters" :value="productsTagsImportResults.success.invalid_data" paginator
                ...
                :globalFilterFields="['data.product_code', 'data.tags']" class="th-table">
                <template #header>
                ...
                <Column field="data.tags" header="Tags" style="min-width: 12rem">
                    <template #body="{ data }">
                        {{ data.data.tags ?? 'No tags' }}
                    </template>
                </Column>
        ...
        <div
            ...
            <DataTable v-model:filters="filters" :value="productsTagsImportResults.success.valid_data" paginator
                ...
                <Column field="taxonomy_item_code" header="Tags" style="min-width: 12rem" />
            </DataTable>
        </div>
    </div>
</template>
