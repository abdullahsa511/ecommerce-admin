...
const handleSave = async () => {
    ...
    try {
        ...
        const response = await productStore.createProductRelationalTableDataImport(formData, 'related_projects');
        importResults.value = {
            success: {
        ...
        console.log('importResults related projects vue :', importResults.value);
        showResults.value = true;
        toast.add({ severity: 'success', summary: 'Success', detail: 'Related projects imported successfully', life: 3000 });
...
<template>
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
        <h2 class="text-xl font-semibold mb-2">Import Related Projects CSV data</h2>
        <p class="text-gray-600 mb-4">Upload a CSV file to import Related Projects data.</p>
        <div class="flex flex-col gap-4">
            <FileUpload mode="basic" name="importRelatedProjectsFile" accept=".csv" :maxFileSize="1000000"
            ...
            <div v-if="selectedFile" class="flex flex-col gap-4">
                ...
                <div v-if="isSaving" class="w-full">
                    ...
                    <p class="text-sm text-gray-600 mt-2 text-center">Importing Related Projects data...</p>
                </div>
                <Button icon="pi pi-save" :label="isSaving ? 'Saving...' : 'Import Related Projects'"
    ...
    <div v-if="showResults && importResults"
        ...
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-semibold">Related Projects Import Results</h2>
        ...
        <div v-if="importResults?.success?.invalid_records > 0 && importResults?.success?.invalid_data"
            ...
            <DataTable v-model:filters="filters" :value="importResults.success.invalid_data" paginator
                :rows="10" dataKey="row" filterDisplay="menu" :globalFilterFields="['data.project_slug', 'data.product_code']"
                class="th-table">
                <template #header>
                    <div class="flex justify-between items-center">
                        <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                        <InputText v-model="filters['global'].value"
                            placeholder="Search invalid related projects records..." />
                    </div>
                </template>
                <template #empty> No invalid related projects records found. </template>
                <Column field="row" header="Row" style="min-width: 6rem">
                    <template #body="{ data }">{{ data.row }}</template>
                </Column>
                <Column field="data.project_slug" header="Project Slug" style="min-width: 10rem">
                    <template #body="{ data }">{{ data.data.project_slug }}</template>
                </Column>
                <Column field="data.product_code" header="Product Code" style="min-width: 12rem">
                    <template #body="{ data }">{{ data.data.product_code }}</template>
                </Column>
                <Column field="data.sort_order" header="Sort Order" style="min-width: 12rem">
        ...
        <div v-if="importResults?.success?.updated_records > 0 && importResults?.success?.updated_data"
            ...
            <DataTable v-model:filters="filters" :value="importResults.success.updated_data" paginator
                :rows="10" dataKey="row" filterDisplay="menu" :globalFilterFields="['project_slug', 'product_code']"
                class="th-table">
                <template #header>
                    <div class="flex justify-between items-center">
                        <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                        <InputText v-model="filters['global'].value" placeholder="Search updated related projects records..." />
                    </div>
                </template>
                <template #empty> No updated related projects records found. </template>
                <Column field="project_slug" header="Project Slug" style="min-width: 10rem">
                        <template #body="{ data }">{{ data.project_slug }}</template>
                </Column>
        ...
        <div
            ...
            <DataTable v-model:filters="filters" :value="importResults.success.valid_data" paginator
                :rows="10" dataKey="project_slug" filterDisplay="menu" :globalFilterFields="['project_slug', 'product_code']"
                class="th-table">
                <template #header>
                    <div class="flex justify-between items-center">
                        <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                        <InputText v-model="filters['global'].value"
                            placeholder="Search valid related projects records..." />
                    </div>
                </template>
                <template #empty> No valid related projects records found. </template>
