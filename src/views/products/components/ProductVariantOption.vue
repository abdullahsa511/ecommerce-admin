...
import Product, { IProductVariant, ProductOption, ProductOptionGroup, ProductVariant } from '@/models/Product.ts';
...
let selectedVariant = ref<IProductVariant>(new ProductVariant({}));
...
const handleUploadImage = async (event: any) => {
  try {
      await productStore.uploadProductVariantImage(event.files, event.productVariantId);
      fb.value.showToast({ success: 'Image uploaded successfully' }, toast);
  } catch (error) {
      fb.value.showToast({ error: 'Error uploading image.'+ error }, toast);
  }
}
...
const searchOptionGroups = async (event: any, variant: IProductVariant) => {
...
const handleSaveVariant = async (data: any) => {
  ...
  try {
    ...
    fb.value.showToast({ error: 'An error occurred while saving the variant.'+ error }, toast);
  }
}
...
const handleAddVariant = async (data: any) => {
  ...
  if (selectedVariant.value.variant_name) {
    const duplicateVariant = productVariants.value.find(
      (variant: ProductVariant) => 
        variant.variant_name?.trim().toLowerCase() === selectedVariant.value.variant_name?.trim().toLowerCase() &&
        variant.product_variant_id !== selectedVariant.value.product_variant_id
    );
    
    if (duplicateVariant) {
      (selectedVariant.value as any).hasDuplicateVariantName = true;
      (selectedVariant.value as any).duplicateWarningMessage = `A variant with the name "${selectedVariant.value.variant_name}" already exists. Please choose a different name.`;
    } else {
      (selectedVariant.value as any).hasDuplicateVariantName = false;
      (selectedVariant.value as any).duplicateWarningMessage = '';
    }
  } else {
    (selectedVariant.value as any).hasDuplicateVariantName = false;
    (selectedVariant.value as any).duplicateWarningMessage = '';
...
const handleDeleteOptionGroup = async (data: any, variant: IProductVariant) => {
  ...
  try {
    ...
    fb.value.showToast({ error: 'Could not delete option group. See console for details.'+ err }, toast);
  }
}
...
const handleInsertOption = async (data: any) => {
  ...
  if (duplicate) {
    fb.value.showToast({ error: `Option added failed. An option named "${option.option_name}" already exists in this group.` }, toast);
    return false;
  }
...
const handleDeleteOption = async (data: any, variant: IProductVariant) => {
  const option = new ProductOption(data);
...
onMounted(async () => {
  // setTimeout(async () => {
    if (props.product.productVariants.length > 0) {
      // update store 
      productVariants.value = props.product.productVariants;
      ...
      await productStore.getVariantsOptionsByProductId(productId as string);
...
<template>
  ...
  <div class="flex flex-col md:flex-row gap-2">
    ...
    <div
      ...
      <VariantForm :fb="fb" v-show="showVariantCard" :productVariants="productVariants"
        :productVariantSuggestions="productVariantSuggestions" :variant="selectedVariant" :buttonLabel="'Save Variant'"
        @searchVariant="searchVariants" @add:variant="handleAddVariant" @insert:variant="handleInsertVariant"
        @submit="(data) => handleSaveVariant(data)" @upload:image="handleUploadImage" />

      <Variant v-for="(item, i) in productVariants" :key="item?.product_variant_id" :index="i + 1" :variant="item"
        :fb="fb" :buttonLabel="'Update Variant'" @update:variant="handleUpdateVariant"
        @delete:variant="handleDeleteVariant" @edit:variant="toggleVariant(item)"
        @click:variant="toggleVariant(item)" @upload:image="handleUploadImage" @searchVariant="searchVariants"/>
