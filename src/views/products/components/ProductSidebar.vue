...
import { ProductSidebarMedia, ProductVariant, ProductImage } from '@/models/Product.ts';
...
const imageThumbnailExpanded = ref(true);
...
const showConfigurationExpanded = ref(true);
...
const statusModel = computed({
    get: () => {
        const status = props.product?.status as unknown;
        if (typeof status === 'string') return status === '1' ? '1' : '0';
    ...
    set: (value: string | number | boolean | null | undefined) => {
        ...
        emit('update:product', { status: isEnabled ? 1 : 0 });
    }
});
...
type ProductImageTypes = 'image' | 'image_banner' | 'banner_image' | 'main_image_one' | 'main_image_two' | 'feature_image_one' | 'feature_image_two' | 'feature_image_three' | 'dimension_image' | 'image_thumb';
...
const deleteSidebarMediaImage = async (file: any, property: ProductSidebarMedia) => {
    try {
...
const removeWayPoint = async (pointId: any) => {
    let response: any = await productStore.removeProductWayPoint(props.product.product_id ?? 0, pointId);
    if (response) {
        toast.add({
            severity: 'success',
            summary: 'Success',
...
const toggleShowConfiguration = () => {
    showConfigurationExpanded.value = !showConfigurationExpanded.value;
};
...
<template>
    <div class="sm:w-full md:w-full lg:w-full xl:w-3/5 space-y-4">
        ...
        <div class="bg-white rounded-lg shadow-sm mt-4">
            <button @click="imageThumbnailExpanded = !imageThumbnailExpanded"
                class="w-full sm:px-6 py-4 text-left text-lg flex justify-between items-center text-primary">
                Image Thumbnail
                <i :class="['pi', imageThumbnailExpanded ? 'pi-chevron-up' : 'pi-chevron-down']"></i>
            </button>

            <div v-if="imageThumbnailExpanded" class="sm:p-6 border-t banner-media-container">
                <!-- Image Preview & Upload -->
                <UploadFileFeature v-model:uploadedFilesData="product.image_thumb" :fb="mediaFb"
        ...
        <div class="bg-white rounded-lg shadow-sm">
            ...
            <div v-if="occanPlasticsExpanded">

                <div class="bg-white rounded-lg shadow-sm flex items-center gap-2 sm:px-6 py-4 mb-4">
                    <label class="block text-left text-lg">Occan Plastics Used</label>
                    <ToggleSwitch v-model="product.ocean_plastic_used" />
                </div>

                <!-- <Checkbox v-model="product.ocean_plastic_used" class="scale-110"
                    :binary="product.ocean_plastic_used ? true : false"
                    :value="product.ocean_plastic_used ? true : false" />
                <label class="ml-2">Occan Plastics Used</label> -->
            </div>
        </div>
        <!-- show configuration section -->
        <div class="bg-white rounded-lg shadow-sm">
            <button @click="toggleShowConfiguration"
                class="w-full sm:px-6 py-4 text-left text-primary text-lg flex justify-between items-center">
                Show Configuration
                <i :class="['pi', showConfigurationExpanded ? 'pi-chevron-up' : 'pi-chevron-down']"></i>
            </button>

            <!-- <div v-if="showConfigurationExpanded" class="p-6 border-t">
                <Checkbox v-model="product.show_configuration" class="scale-110" :binary="product.show_configuration ? true : false" :value="product.show_configuration ? true : false" />
                <label class="ml-2">Show Configuration</label>
                <ToggleSwitch v-model="product.show_configuration" />
            </div> -->
            <div class="bg-white rounded-lg shadow-sm flex items-center gap-2 sm:px-6 py-4" v-if="showConfigurationExpanded">
                <label class="block text-left text-lg">Show Configuration on Product Page</label>
                <ToggleSwitch v-model="product.show_configurator" />
            </div>
        </div>
    </div>
        ...
        <Dialog v-model:visible="visible" modal header="Setup Way-Points" maximizable :maximized="true"
            ...
            <div class="body-container">
                <WayPoint :wayPointSuggestions="wayPointSuggestions" :imageSrc="resolveMediaImage(product.banner_image)"
                    :wayPoints="bannerWayPointsData" :modelId="product.product_id" :fb="mediaFb"
