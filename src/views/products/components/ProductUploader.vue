  ...
  <div class="flex flex-col md:flex-row gap-2">
    ...
    <div class="md:w-2/3 w-full bg-white rounded-xl shadow p-6 flex flex-col gap-6 border border-gray-200">
      ...
      <div>
        ...
        <label class="flex flex-col items-center justify-center border-2 border-dashed border-blue-400 rounded-lg p-6 cursor-pointer hover:bg-blue-50 transition">
      ...
      <div>
        ...
        <input v-model="form.name" class="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-300 outline-none" />
      </div>
      <!-- Description -->
      <div>
        <label class="block font-semibold text-gray-700 mb-1">Description</label>
        <textarea v-model="form.description" class="w-full border border-gray-300 rounded-md p-2 min-h-[80px] focus:ring-2 focus:ring-blue-300 outline-none" />
      </div>
      <!-- Metadata -->
      <div>
        <label class="block font-semibold text-gray-700 mb-1">Metadata</label>
        <input v-model="form.metadata" class="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-300 outline-none" />
      </div>
      <!-- Tags -->
      <div>
        <label class="block font-semibold text-gray-700 mb-1">Tags</label>
        <input v-model="form.tags" class="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-300 outline-none" />
      </div>
    </div>
    ...
    <div class="md:w-1/3 w-full bg-white rounded-xl shadow p-6 border border-gray-200 flex flex-col gap-6">
      <div v-if="uploaded">
        ...
        <div>
          ...
          <div class="flex items-start mb-4">
            ...
            <textarea class="border border-gray-300 p-2 w-64 rounded bg-gray-50" rows="2" readonly>{{ form.armsVariance }}</textarea>
          </div>
      ...
      <div v-else class="flex flex-col items-center justify-center h-full text-gray-400">
        <svg class="w-16 h-16 mb-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 16v-8m0 0l-3 3m3-3l3 3m-9 4a9 9 0 1118 0 9 9 0 01-18 0z" />
        </svg>
