...
const relatedFamilySuggestions = ref<any[]>([]);
const relatedResourceSuggestions = ref<any[]>([]);
const relatedProjectSuggestions = ref<any[]>([]);
...
const showRelatedProductAutocomplete = ref(false);
const showRelatedFamilyAutocomplete = ref(false);
const showRelatedProjectAutocomplete = ref(false);
const showRelatedResourceAutocomplete = ref(false);
const showDigitalAssetAutocomplete = ref(false);

function initiateRelatedProductAutocomplete() {
  showRelatedFamilyAutocomplete.value = false;
  showRelatedProductAutocomplete.value = true;
  nextTick(() => {
...
function initiateRelatedFamilyAutocomplete() {
  showRelatedProductAutocomplete.value = false;
  showRelatedFamilyAutocomplete.value = true;
  nextTick(() => {
    const input = document.querySelector('.related-family-autocomplete > .p-autocomplete-input');
    if (input instanceof HTMLElement) {
      input.focus();
    }
  });
}
function initiateRelatedProjectAutocomplete() {
  showRelatedProductAutocomplete.value = false;
  showRelatedProjectAutocomplete.value = true;
  nextTick(() => {
    const input = document.querySelector('.related-project-autocomplete > .p-autocomplete-input');
    if (input instanceof HTMLElement) {
      input.focus();
    }
  });
...
function initiateRelatedResourceAutocomplete() {
  showRelatedProductAutocomplete.value = false;
  showRelatedResourceAutocomplete.value = true;

  nextTick(() => {
    const input = document.querySelector(
      '.related-resource-autocomplete > .p-autocomplete-input'
    );
...
const getRelatedFamilyList = () => {
  const productWithRelatedFamily = props.product as IProduct & { relatedFamily?: RelatedFamilyItem[] };
  if (!Array.isArray(productWithRelatedFamily.relatedFamily)) {
    productWithRelatedFamily.relatedFamily = [];
  }
  return productWithRelatedFamily.relatedFamily;
};

const searchRelatedFamilies = (event: { query: string }) => {
  const { query } = event;
  if (query?.length > 0) {
    productStore.searchProducts(query).then(results => {
      relatedProductSuggestions.value = results;
...
function addRelatedProductFamily(event: { value: IProduct }) {
  if (!props.product.familyProducts.some(p => p.product_id === event.value.product_id)) {
    props.product.familyProducts.push(event.value);
  }
  // Reset the autocomplete
  showRelatedFamilyAutocomplete.value = false;
  nextTick(() => {
...
function removeRelatedFamily(idx: number) {
  // product id
  const productId = props.product.product_id;
  // related product id
  const familyProductId = props.product.familyProducts[idx].product_id;
  ...
  if (confirmed) {
    productStore.removeProductFromFamily(productId as number, familyProductId as number).then(response => {
      console.log("response", response);
      toast.add({ severity: 'success', summary: 'Success', detail: 'Successfully removed the product from family', life: 3000 }); 
      props.product.familyProducts.splice(idx, 1);
    }).catch(error => {
...
//   const productWithRelatedProjects = props.product as IProduct & {
//     relatedProjects?: IProject[];
//   };

//   if (!Array.isArray(productWithRelatedProjects.relatedProjects)) {
//     productWithRelatedProjects.relatedProjects = [];
//   }

//   return productWithRelatedProjects.relatedProjects;
// };

const searchRelatedProjects = (event: { query: string }) => {
  const { query } = event;

  if (query?.length > 0) {
    productStore.searchProjects(query).then(results => {
      relatedProjectSuggestions.value = results;
    });
  } else {
    relatedProjectSuggestions.value = [];
  }
};

function addRelatedProject(event: { value: any }) {
  if (
    !props.product.relatedProjects?.some(
      p => p.project_id === event.value.project_id
    )
  ) {
    props.product.relatedProjects.unshift(event.value);
  }

  showRelatedProjectAutocomplete.value = false;
  ...
  nextTick(() => {
    ...
    nextTick(() => {
      const input = document.querySelector(
        '.related-project-autocomplete > .p-autocomplete-input'
      );
...
function removeRelatedProject(idx: number) {
  const productId = props.product.product_id;
  const projectId = props.product.relatedProjects[idx].project_id;
  ...
  if (confirmed) {
    productStore
      .removeRelatedProject(productId as number, projectId as number)
      .then(() => {
        toast.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Successfully removed related project',
          life: 3000,
        });

        props.product.relatedProjects.splice(idx, 1);
      })
      .catch(() => {
        toast.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to delete related project',
          life: 3000,
        });
      });
  }
...
const searchRelatedResources = (event: { query: string }) => {
  const { query } = event;

  if (query?.length > 0) {
    productStore.searchResources(query).then(results => {
      relatedResourceSuggestions.value = results;
    });
  } else {
    relatedResourceSuggestions.value = [];
  }
};

function addRelatedResource(event: { value: any }) {
  if (
    !props.product.relatedResources?.some(
      r => r.design_resource_id === event.value.design_resource_id
    )
  ) {
    props.product.relatedResources.push(event.value);
  }

  showRelatedResourceAutocomplete.value = false;
...
function removeRelatedResource(idx: number) {
  const productId = props.product.product_id;
  const resourceId = props.product.relatedResources[idx].resource_id;

  const confirmed = window.confirm(
    'Are you sure you want to delete this related resource?'
  );

  if (confirmed) {
    productStore
      .removeRelatedResource(productId as number, resourceId as number)
      .then(() => {
        toast.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Successfully removed related resource',
          life: 3000,
        });

        props.product.relatedResources.splice(idx, 1);
      })
      .catch(() => {
        toast.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to delete related resource',
          life: 3000,
        });
      });
  }
...
// 1. Define your static list of certificates
const staticCertificates = ['AFRDI Green', 'AFRDI Blue'];

const certificateSuggestions = ref<string[]>([...staticCertificates]);

const searchCertificates = (event: { query: string }) => {
    const query = event.query.toLowerCase().trim();
    
    if (!query) {
      certificateSuggestions.value = [...staticCertificates];
    } else {
      certificateSuggestions.value = staticCertificates.filter(cert => 
    ...
    <div class="flex gap-6" v-if="!fb.loading['product.show' + product.product_id]">
      ...
      <div class="w-3/4 space-y-6">
        ...
        <div class="space-y-8">
          ...
          <!-- <div class="space-y-2 w-full">
            <h3 class="font-medium text-lg">Product Certificates</h3>
            <AutoComplete 
                ref="certificatesAutocomplete" 
                v-model="product.certificates"
                :suggestions="certificateSuggestions"
                @complete="searchCertificates" 
                multiple 
                placeholder="Type to search or add certificates" 
                class="w-full text-lg"
                inputClass="w-full !rounded-md !border !border-gray-200 !shadow-none !focus:ring-0" 
            />
        </div> -->
          ...
          <div id="related-product" class="space-y-3">
            ...
            <div class="flex items-center justify-between w-full">
              ...
              <Button label="Add" icon="pi pi-plus" class="p-button-text text-blue-600 hover:text-blue-800"
                @click="initiateRelatedProductAutocomplete" />
            </div>

            <!-- Autocomplete -->
            <AutoComplete v-if="showRelatedProductAutocomplete" ref="productAutocomplete"
          ...
          <!-- start related family -->
          <div id="related-family" class="space-y-3">
            <!-- Header -->
            <div class="flex items-center justify-between w-full">
              <h3 class="font-medium text-lg text-gray-800">Related Family</h3>
              <Button label="Add" icon="pi pi-plus" class="p-button-text text-blue-600 hover:text-blue-800"
                @click="initiateRelatedFamilyAutocomplete" />
            </div>

            <!-- Autocomplete -->
            <AutoComplete v-if="showRelatedFamilyAutocomplete" ref="productAutocomplete"
              :suggestions="relatedProductSuggestions" @complete="searchRelatedFamilies" @item-select="addRelatedProductFamily"
              optionLabel="name" placeholder="Type for autocomplete" class="w-full related-family-autocomplete"
              inputClass="w-full !rounded-md !border !border-gray-200 !shadow-none !focus:ring-0" />

            <!-- Related Product Cards -->
            <div class="bg-white border border-gray-200 rounded p-3" style="min-height: 48px;">
              <!-- Grid of Cards -->
              <div v-if="product.familyProducts && product.familyProducts.length"
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                <div v-for="(productItem, idx) in product.familyProducts" :key="idx"
                  class="relative border border-gray-100 shadow-sm hover:shadow-md transition bg-white overflow-hidden">
                  <!-- Remove Button -->
                  <button @click="removeRelatedFamily(idx)"
                    class="absolute top-0 right-0 bg-gray-500 text-white hover:bg-black  w-6 h-6 flex items-center justify-center shadow-sm"
                    aria-label="Remove" type="button">
                    ×
                  </button>
          ...
          <div id="related-project" class="space-y-3">
            <div class="flex items-center justify-between w-full">
              <h3 class="font-medium text-lg text-gray-800">Related Projects</h3>
              <Button
                label="Add"
                icon="pi pi-plus"
                class="p-button-text text-blue-600 hover:text-blue-800"
                @click="initiateRelatedProjectAutocomplete"
              />
            </div>

            <!-- Autocomplete -->
            <AutoComplete
              v-if="showRelatedProjectAutocomplete"
              ref="projectAutocomplete"
              :suggestions="relatedProjectSuggestions"
              @complete="searchRelatedProjects"
              @item-select="addRelatedProject"
              optionLabel="name"
              placeholder="Type for autocomplete"
              class="w-full related-project-autocomplete"
              inputClass="w-full !rounded-md !border !border-gray-200 !shadow-none !focus:ring-0"
            />
            ...
            <div class="bg-white border border-gray-200 rounded p-3" style="min-height: 48px;">
              <div
                v-if="product.relatedProjects && product.relatedProjects.length"
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4"
              >
                <div
                  v-for="(projectItem, idx) in product.relatedProjects"
                  :key="idx"
                  class="relative border border-gray-100 shadow-sm hover:shadow-md transition bg-white overflow-hidden"
                >
                  <button
                    @click="removeRelatedProject(idx)"
                    class="absolute top-0 right-0 bg-gray-500 text-white hover:bg-black w-6 h-6 flex items-center justify-center shadow-sm"
                    type="button"
                  >
                    ×
                  ...
                  <div class="p-3 text-left mb-3">
                    <p class="truncate text-lg">{{ projectItem.name }}</p>
                  </div>
                </div>
              </div>
              ...
              <div
                v-else
                class="text-gray-400 text-sm text-center py-4 border border-dashed border-gray-200 rounded"
              >
                No related projects selected.
              </div>
            </div>
          </div>
          <!-- end related project -->
          ...
          <!-- <div id="related-resource" class="space-y-3">
            <div class="flex items-center justify-between w-full">
              <h3 class="font-medium text-lg text-gray-800">Related Resources</h3>
              <Button
                label="Add"
                icon="pi pi-plus"
                class="p-button-text text-blue-600 hover:text-blue-800"
                @click="initiateRelatedResourceAutocomplete"
              />
            </div>
              <AutoComplete
                v-if="showRelatedResourceAutocomplete"
                ref="resourceAutocomplete"
                :suggestions="relatedResourceSuggestions"
                @complete="searchRelatedResources"
                @item-select="addRelatedResource"
                optionLabel="name"
                placeholder="Type for autocomplete"
                class="w-full related-resource-autocomplete"
                inputClass="w-full !rounded-md !border !border-gray-200 !shadow-none !focus:ring-0"
              />
              ...
              <div class="bg-white border border-gray-200 rounded p-3" style="min-height: 48px;">
                <div
                  v-if="product.relatedResources && product.relatedResources.length"
                  class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4"
                >
                  <div
                    v-for="(resourceItem, idx) in product.relatedResources"
                    :key="idx"
                    class="relative border border-gray-100 shadow-sm hover:shadow-md transition bg-white overflow-hidden"
                  >
                    <button
                      @click="removeRelatedResource(idx)"
                      class="absolute top-0 right-0 bg-gray-500 text-white hover:bg-black w-6 h-6 flex items-center justify-center shadow-sm"
                      type="button"
                    >
                      ×
                    ...
                    <div class="p-3 text-left mb-3">
                      <p class="truncate text-lg">{{ resourceItem.name }}</p>
                    </div>
                    <div class="p-3 text-left mb-3">
                      <p class="truncate text-lg">{{ resourceItem.type }}</p>
                    </div>
                  </div>
                </div>
