<script setup lang="ts">
  ...
  const props = defineProps<{
    /** Variant id from `edit/:id` (vue-router `props: true` on this route). */
    id?: string;
  }>();

  const route = useRoute();

  /** Prefer the declared `id` prop so router does not pass an extraneous non-prop attribute. */
  const routeVariantId = computed(() => {
    if (props.id != null && props.id !== '') return props.id;
  ...
  const handleSaveVariant = async (data: any) => {
    ...
    try {
      const parentProductId =
        productVariants.value[0]?.product_id ??
        selectedVariant.value?.product_id ??
        product.value?.product_id;
      data.product_id = parentProductId as number;
      let response: any = null;
      response = await productStore.createProductVariant(data);
      if (response) {
        // selectedVariant.value = productVariants.value[productVariants.value.length - 1];
        selectedVariant.value = productVariants.value[0];
        // newVariant.value = new ProductVariant(data);
  ...
  const handleAddVariant = async (data: any) => {
    ...
    const pid =
      ...
      product.value?.product_id ??
      selectedVariant.value?.product_id;
    selectedVariant.value.product_id = pid as number;
  ...
  const handleUploadImage = async (event: any, property: ProductImageTypes) => {
    try {
        let productVariantId = selectedVariant.value?.product_variant_id ? selectedVariant.value?.product_variant_id : 0; // 0 is for new products
        const files: FileModel[] = await productStore.uploadProductVariantImage(event.files, productVariantId as number);
        (selectedVariant.value as any)[property] = files;
    } catch (error) {
        console.error('Error uploading site logo:', error);
    }
  }

  const handleRemoveUploadedFile = async (file: any, property: ProductImageTypes) => {
    try {
        // await mediaStore.deleteImageByFilePath(file.objectURL, property);
        const productVariantId = selectedVariant.value?.product_variant_id ? selectedVariant.value?.product_variant_id : 0; // 0 is for new products
        const deleteFile = await productStore.deleteProductVariantImage(productVariantId as number);
        (selectedVariant.value as any)[property] = [];
        toast.add({
            severity: 'success',
            summary: 'Success',
            detail: 'Image deleted successfully',
  ...
  onMounted(async () => {
    await productStore.getVariantById(routeVariantId.value as string);
  ...
  async function submitFromParent() {
    ...
    if (v?.isNew || v?.product_variant_id == null) {
      await handleSaveVariant(v);
      return;
    }
    await handleUpdateVariant(v);
  }

  const handleUploadOptionImage = async (event: any, property: ProductImageTypes) => {
    try {
        const productOptionId = event.productOptionId;
        const uploadedFiles: FileModel[] | undefined = await productStore.uploadProductVariantGroupOptionImage(
          event.files,
          productOptionId as number
        );
        if (!uploadedFiles?.length) return;

        const groups = selectedVariant.value?.productOptionGroups;
        if (!groups?.length) return;

        for (const group of groups) {
          const opt = group.productOptions?.find(
            (o) => o.product_option_id === productOptionId
          );
          if (!opt) continue;
          (opt as any)[property] = uploadedFiles;
          // opt.imagePreview = uploadedFiles[0]?.objectURL ?? '';
          break;
        }
        toast.add({
            severity: 'success',
  ...
  const handleRemoveOptionImage = async (event: any, property: ProductImageTypes) => {
    try {
      const productOptionId = event.productOptionId;
      await productStore.deleteProductVariantOptionImage(productOptionId as number);
      const group = selectedVariant.value?.productOptionGroups.find(g => g.productOptions?.find(o => o.product_option_id === productOptionId));
      if (group) {
        const option = group.productOptions?.find(o => o.product_option_id === productOptionId);
        if (option) {
          option.image = [];
        }
      }
...
<template>
      <Message 
      v-if="fb.errors?.['product.variant.update.' + selectedVariant?.product_variant_id]?.global_message 
      || fb.errors?.['product.variant.create']?.global_message" class="br_content border-l-4 mb-5 w-full rounded-md shadow-sm">
        <span v-for="error in fb.errors?.['product.variant.update.' + selectedVariant?.product_variant_id]?.global_message || fb.errors?.['product.variant.create']?.global_message" :key="error" class="flex flex-col">
          {{ error }}
        </span>  
      </Message>
    ...
    <div class="flex flex-col md:flex-row gap-2">
      ...
      <div
        ...
        <VariantForm 
          ...
          @submit="(data) => handleUpdateVariant(data)" 
          @upload:image="handleUploadImage($event, 'image')"
          @remove:image="handleRemoveUploadedFile($event, 'image')"
          />
      ...
      <div ref="optionGroupsHeadingRef"
        ...
        <OptionGroupForm 
          ...
          @searchTypes="searchTypes($event)" @add:type="onSelectType($event)" />
  
        <OptionGroupList :variant="selectedVariant"
          :fb="fb"
          ...
          @searchOptionGroups="searchOptionGroups($event, selectedVariant)"
          @searchOptions="searchOptions($event)" @add:option="handleAddOption" @insert:option="handleInsertOption"
          @update:option="handleUpdateOption"
          @delete:option="handleDeleteOption($event, selectedVariant)"
          @searchTypes="searchTypes($event)" @add:type="onSelectType($event)" 
          @upload:optionImage="handleUploadOptionImage($event, 'image')"
          @remove:optionImage="handleRemoveOptionImage($event, 'image')"
          />
