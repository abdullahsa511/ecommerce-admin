<script setup lang="ts">
  ...
  const optionGroup = ref<ProductOptionGroup>(new ProductOptionGroup({product_id: product.value.product_id}));
  ...
  const handleDeleteVariant = async (data: any) => {
    ...
    // selectedVariant.value = productVariants.value[0];
    optionGroup.value = new ProductOptionGroup({product_id: product.value.product_id});
  }
  ...
  const handleSaveVariant = async (data: any) => {
    ...
    // CHECK DUPLICATES OPTION GROUP 
    // const normalizedGroupName = data.option_group_name.trim().toLowerCase();
  
    // const checkExistingOptionGroup = selectedVariant.value?.productOptionGroups?.some(
    //   (group: any) =>
    //     group.option_group_name.trim().toLowerCase() === normalizedGroupName
    // );
  
    // if (checkExistingOptionGroup) {
    //   toast.add({
    //     severity: 'error',
    //     summary: 'Duplicate Option Group',
    //     detail: `An option group named "${data.option_group_name}" already exists for this variant.`,
    //     life: 10000
    //   });
    //   return;
    // }
   // END CHECK DUPLICATES OPTION GROUP 
    try {
  ...
  const handleInsertOptionGroup = async (group: ProductOptionGroup) => {
    ...
    selectedVariant.value.product_id = product.value.product_id;
    ...
    if (groupIndex !== undefined && groupIndex !== null && groupIndex > -1) {
      if (variant) {
        ...
        try {
          ...
          fb.showToast({error: 'Failed to delete product option.'});
        }
      }
    }
  }
  ...
  onMounted(async () => {
    await productStore.getVariantById(741);
