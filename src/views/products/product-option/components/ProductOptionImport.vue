...
const handleSaveOptions = async () => {
  ...
  try {
    ...
    const response = await productOptionStore.productOptionsImport(formData);
...
<template>
  <div class="option-import p-6">
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">Import Product Options CSV data</h2>
      <p class="text-gray-600 mb-4">Upload a CSV file to start importing product options data</p>
      <div class="flex flex-col gap-4">
        <FileUpload mode="basic" name="importOptionsFile" accept=".csv" :maxFileSize="10000000"
        ...
        <div v-if="selectedFileOptions" class="flex flex-col gap-4">
          ...
          <Button icon="pi pi-save" :label="isSavingOptions ? 'Saving...' : 'Import Product Options'" severity="success"
      ...
      <div v-if="showOptionsResults && optionsImportResults" class="bg-white rounded-lg p-6 shadow-md mt-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-semibold">Product Options Import Results</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <div class="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <div class="text-2xl font-bold text-purple-600">{{ optionsImportResults.inserted_records || 0 }}</div>
        ...
        <div v-if="optionsImportResults.invalid_records > 0 && optionsImportResults.invalid_data" class="mb-6">
          ...
          <DataTable v-model:filters="invalidFilters" :value="optionsImportResults.invalid_data" paginator :rows="100"
            dataKey="row" filterDisplay="menu"
            :globalFilterFields="['data.web_product', 'data.product_variant', 'data.option_group', 'data.option', 'data.sort_order']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="invalidFilters['global'].value" placeholder="Search invalid product options records..." />
              </div>
            </template>
            ...
            <Column field="data.web_product" header="Product" style="min-width: 10rem"/>
            <Column field="data.product_variant" header="Variant" style="min-width: 10rem"/>
            <Column field="data.option_group" header="Option Group" style="min-width: 15rem"/>
            <Column field="data.option" header="Option" style="min-width: 15rem"/>
            <Column field="data.sort_order" header="Sort Order" style="min-width: 5rem"/>
          </DataTable>
        </div>

        <!-- Duplicate Records DataTable -->
        <div v-if="optionsImportResults.duplicate_records > 0 && optionsImportResults.duplicate_data" class="mb-6">
          <h3 class="text-lg font-semibold mb-4 text-orange-700">
            Duplicate Records ({{ optionsImportResults.duplicate_records }})
          </h3>
          <DataTable v-model:filters="duplicateFilters" :value="optionsImportResults.duplicate_data" paginator :rows="100"
            dataKey="option_id" filterDisplay="menu"
            :globalFilterFields="['web_product', 'product_variant', 'option_group', 'option', 'sort_order']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="duplicateFilters['global'].value" placeholder="Search duplicate product options records..." />
              </div>
            </template>
            <template #empty>No duplicate product options records found.</template>
        ...
        <div v-if="optionsImportResults.updated_records > 0 && optionsImportResults.updated_data" class="mb-6">
          ...
          <DataTable v-model:filters="updatedFilters" :value="optionsImportResults.updated_data" paginator :rows="100"
            dataKey="option_id" filterDisplay="menu"
            :globalFilterFields="['web_product', 'product_variant', 'option_group', 'option', 'sort_order']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
        ...
        <div v-if="optionsImportResults.inserted_records > 0 && optionsImportResults.inserted_data">
          <h3 class="text-lg font-semibold mb-4 text-green-700">
            Valid Records ({{ optionsImportResults.inserted_records }})
          </h3>
          <DataTable v-model:filters="insertedFilters" :value="optionsImportResults.inserted_data" paginator :rows="100"
            dataKey="option" filterDisplay="menu" :globalFilterFields="['web_product', 'product_variant', 'option_group', 'option', 'sort_order']"
            class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="insertedFilters['global'].value" placeholder="Search valid product option content records..." />
              </div>
            </template>
            <template #empty>No valid product option content records found.</template>
            <Column field="web_product" header="Product" style="min-width: 15rem" />
            <Column field="product_variant" header="Variant" style="min-width: 15rem" />
            <Column field="option_group" header="Option Group" style="min-width: 15rem" />
            <Column field="option" header="Option" style="min-width: 15rem" />
