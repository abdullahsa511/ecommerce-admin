    ...
    <template>
      <div class="variant-import p-6">
        ...
        <div class="bg-white rounded-lg p-6 shadow-md">
          ...
          <p class="text-gray-600 mb-4">Upload a CSV file to start importing product tag data</p>
          <div class="flex flex-col gap-4">
            <FileUpload mode="basic" name="importProductTagFile" accept=".csv" :maxFileSize="1000000" @select="handleFileSelect"
            ...
            <div v-if="selectedFileVendors" class="flex flex-col gap-4">
              ...
              <div v-if="fb.loading['productTag.import']" class="w-full">
                <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
                <p class="text-sm text-gray-600 mt-2 text-center">Importing product tag data...</p>
              </div>
              <Button icon="pi pi-save" :label="fb.loading['productTag.import'] ? 'Saving...' : 'Import product tag'"
                severity="success" @click="handleSave" :loading="fb.loading['productTag.import']"
                :disabled="fb.loading['productTag.import']" class="p-button-md" />
            </div>
          </div>
          ...
          <div v-if="showVendorResults" class="bg-white rounded-lg p-6 shadow-md mt-6">
              ...
              <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="100" dataKey="row"
                ...
                <template #header>
                  <div class="flex justify-between items-center">
                    ...
                    <InputText v-model="filters['global'].value" placeholder="Search invalid product tag records..." />
                  </div>
                </template>
                <template #empty>No invalid product tag records found.</template>
                ...
                <Column field="data.slug" header="Slug" style="min-width: 10rem">
                  <template #body="{ data }">
                    {{ data.data.slug }}
                  </template>
                </Column>
            ...
            <div v-if="importExport.duplicated_records > 0 && importExport.duplicated_data" class="mb-6">
              ...
              <DataTable 
                ...
                <template #header>
                  <div class="flex justify-between items-center">
                    <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                    <InputText v-model="filters['global'].value" placeholder="Search duplicate product tags records..." />
                  </div>
                </template>
                ...
                <Column field="slug" header="Slug" style="min-width: 10rem">
                  <template #body="{ data }">{{ data.slug }}</template>
                </Column>
              </DataTable>
            </div>
            ...
            <div v-if="importExport.updated_records > 0 && importExport.updated_data" class="mb-6">
              ...
              <DataTable v-model:filters="filters" :value="importExport.updated_data" paginator :rows="100"
                dataKey="tag_id" filterDisplay="menu"
                :globalFilterFields="['name', 'vendor_code', 'sort_order']" class="th-table">
                <template #header>
                  <div class="flex justify-between items-center">
                    <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                    <InputText v-model="filters['global'].value" placeholder="Search updated product tags records..." />
                  </div>
                </template>
            ...
            <div v-if="importExport.valid_records > 0 && importExport.valid_data">
              ...
              <DataTable v-model:filters="filters" :value="importExport.valid_data" paginator :rows="100" dataKey="tag_id"
                filterDisplay="menu" :globalFilterFields="['name', 'slug']" class="th-table">
                <template #header>
                  <div class="flex justify-between items-center">
                    <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                    <InputText v-model="filters['global'].value" placeholder="Search valid product tags records..." />
                  </div>
                </template>
                <template #empty>No valid product tags records found.</template>
              
                <Column field="name" header="Name" style="min-width: 15rem" />
                <Column field="slug" header="Slug" style="min-width: 15rem" />
              </DataTable>
            </div>
