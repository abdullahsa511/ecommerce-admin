  ...
  <div class="flex flex-col gap-8">
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
      ...
      <div class="flex flex-col gap-4">
        <FileUpload mode="basic" name="importCategoriesFile" accept=".csv" :maxFileSize="1000000"
          @select="handleFileSelectCategories" :auto="false" chooseLabel="Select CSV File" />
        ...
        <div v-if="selectedFileCategories" class="flex flex-col gap-4">
          ...
          <Button icon="pi pi-save" :label="isSavingCategories ? 'Saving...' : 'Import Categories'" severity="success"
            @click="handleSaveCategories" :loading="isSavingCategories" :disabled="isSavingCategories"
            class="p-button-md" />
        </div>
      </div>
    </div>
    ...
    <div v-if="showCategoriesResults && categoriesImportResults" class="bg-white rounded-lg p-6 shadow-md">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold">Product Tags Import Results</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
          <div class="text-2xl font-bold text-blue-600">{{ categoriesImportResults?.success?.total_records || 0 }}</div>
          <div class="text-sm text-blue-800">Total Records</div>
        </div>
        <div class="bg-green-50 p-4 rounded-lg border border-green-200">
          <div class="text-2xl font-bold text-green-600">{{ categoriesImportResults?.success?.valid_records || 0 }}
          </div>
          <div class="text-sm text-green-800">Valid Records</div>
        </div>
        <div class="bg-red-50 p-4 rounded-lg border border-red-200">
          <div class="text-2xl font-bold text-red-600">{{ categoriesImportResults?.success?.invalid_records || 0 }}
          </div>
          <div class="text-sm text-red-800">Invalid Records</div>
        </div>
        <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
          <div class="text-2xl font-bold text-orange-600">{{ categoriesImportResults?.success?.updated_records || 0 }}
          </div>
          <div class="text-sm text-orange-800">Updated Records</div>
        </div>
        <div class="bg-purple-50 p-4 rounded-lg border border-purple-200">
          <div class="text-2xl font-bold text-purple-600">{{ categoriesImportResults?.success?.inserted_count || 0 }}
          </div>
      ...
      <div v-if="categoriesImportResults?.success?.categories" class="mb-8">
        ...
        <!-- Invalid Categories -->
        <div
          v-if="categoriesImportResults.success.categories.invalid_data && categoriesImportResults.success.categories.invalid_data.length > 0"
          class="mb-6">
          <h4 class="text-md font-semibold mb-3 text-red-600">Invalid Categories ({{
            categoriesImportResults.success.categories.invalid_data.length }})</h4>
          <DataTable v-model:filters="filters" :value="categoriesImportResults.success.categories.invalid_data"
            paginator :rows="10" dataKey="row" filterDisplay="menu"
            :globalFilterFields="['data.category_name', 'data.category_link', 'data.meta_keywords']" class="th-table">
            <template #header>
        ...
        <!-- Valid Categories -->
        <div
          v-if="categoriesImportResults.success.categories.valid_data && categoriesImportResults.success.categories.valid_data.length > 0">
          <h4 class="text-md font-semibold mb-3 text-green-600">
            Valid Categories ({{ categoriesImportResults.success.categories.valid_data.length }})
          </h4>
          <DataTable v-model:filters="filters" :value="categoriesImportResults.success.categories.valid_data" paginator
            :rows="10" dataKey="taxonomy_item_id" filterDisplay="menu"
            :globalFilterFields="['data.name', 'data.taxonomy_item_code']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            ...
            <Column field="taxonomy_item_code" header="Link" style="min-width: 15rem" />
            <!-- <Column field="template" header="Template" style="min-width: 20rem" /> -->
          </DataTable>
        </div>
      ...
      <div v-if="categoriesImportResults?.success?.sub_categories">
        <h3 class="text-lg font-semibold mb-4 text-indigo-700">Sub-Categories ({{
          categoriesImportResults.success.sub_categories.length }})</h3>

        <!-- Invalid Sub-Categories -->
        <div
          v-if="categoriesImportResults.success.sub_categories.invalid_data && categoriesImportResults.success.sub_categories.invalid_data.length > 0"
          class="mb-6">
          <h4 class="text-md font-semibold mb-3 text-red-600">Invalid Sub-Categories ({{
            categoriesImportResults.success.sub_categories.invalid_data.length }})</h4>
          <DataTable v-model:filters="filters" :value="categoriesImportResults.success.sub_categories.invalid_data"
            paginator :rows="10" dataKey="row" filterDisplay="menu"
            :globalFilterFields="['data.sub_category_name', 'data.sub_category_link', 'data.parent_category_id', 'data.meta_keywords']"
            class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
        ...
        <div
          v-if="categoriesImportResults.success.subcategories.valid_data && categoriesImportResults.success.subcategories.valid_data.length > 0">
          <h4 class="text-md font-semibold mb-3 text-green-600">Valid Sub-Categories ({{
            categoriesImportResults.success.subcategories.valid_data.length }})</h4>
          <DataTable v-model:filters="filters" :value="categoriesImportResults.success.subcategories.valid_data"
            paginator :rows="10" dataKey="sub_category_id" filterDisplay="menu"
            :globalFilterFields="['sub_category_name', 'sub_category_link', 'parent_category_id', 'meta_keywords']"
            class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
