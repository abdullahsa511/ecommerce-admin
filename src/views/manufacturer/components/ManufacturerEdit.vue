...
const emit = defineEmits(['update:manufacturer', 'saved', 'editTabUnmunted', 'editTabMounted']);
...
const uploadManufacturerImages = async (event: any) => { // property: 'image'
    try {
        let files = event.files;
        files = files.map((file: any) => {
            let f = manufacturer.value.image.find((img: any) => img.name === file.file?.name);
            if (f) file.id = f.id;
            return file;
        });
        await manufacturerStore.uploadManufacturerImages(files, 'image', manufacturer.value.manufacturer_id);
    } catch (error) {
        console.error('Error uploading image:', error);
    }
}

const deleteManufacturerImages = async (file: any) => {
    try {
        await manufacturerStore.deleteManufacturerImages(file, manufacturer.value.manufacturer_id);
...
onMounted(async () => {
    emit('editTabMounted');
    if (route.params.id) {
        await manufacturerStore.getManufacturerById(route.params.id as string);
    }
...
<template>
    <div class="flex flex-col gap-6">
        <div class="loading-overlay border-2" v-if="fb.loading['manufacturer.show.' + route.params.id]">
            <div class="flex justify-center items-center h-full spinner">
                <i class="pi pi-spin pi-spinner" style="font-size: 2rem;"></i>
                <p class="mt-2">Loading manufacturer details. Please wait.</p>
            </div>
        </div>
            ...
            <div class="sm:w-full md:w-3/5 space-y-4">
                ...
                <div class="grid grid-cols-12 gap-x-6 gap-y-4">
                    ...
                    <div class="col-span-12 md:col-span-9 xl:col-span-10 py-2">
                        <InputText v-model="manufacturer.manufacturer_code"
                    ...
                    <div class="col-span-12 md:col-span-9 xl:col-span-10 py-2">
                        <InputText v-model="manufacturer.name"
                            class="w-full text-lg border border-gray-200 rounded-md p-2"
                    ...
                    <div class="col-span-12 md:col-span-9 xl:col-span-10 py-2">
                        <InputText v-model="manufacturer.slug"
                            class="w-full text-lg border border-gray-200 rounded-md p-2"
                    ...
                    <div class="col-span-12 md:col-span-9 xl:col-span-10 py-2">
                        <InputNumber v-model="manufacturer.sort_order"
                            class="w-full text-lg border border-gray-200 rounded-md p-2"
                ...
                <div class="bg-white rounded-lg shadow-sm mt-4">
                    ...
                    <div v-if="imageMediaExpanded" class="sm:p-6 border-t">
                        <UploadFileFeature v-model:uploadedFilesData="manufacturer.image" :fb="mediaFb"
                            :property="'image'" @upload="uploadManufacturerImages($event)"
                            @removeUploadedFile="deleteManufacturerImages($event)" />
                    </div>
                </div>
            </div>
        </div>
