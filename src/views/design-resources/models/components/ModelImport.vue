...
const handleSave = async () => {
  ...
  try {
    ...
    const response = await designResourceImportStore.createModelsImport(formData);

    // Store the import results
    importResults.value = response;
    showResults.value = true;

    toast.add({ severity: 'success', summary: 'Success', detail: 'Models data imported successfully', life: 3000 });
    selectedFile.value = null;
  } catch (error) {
    console.error('Save error:', error);
    toast.add({ severity: 'error', summary: 'Error', detail: error.message || 'Failed to import models data', life: 3000 });
  } finally {
...
<template>
  <div class="flex flex-col gap-8">
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
      ...
      <p class="text-gray-600 mb-4">Upload a CSV file to start importing models data</p>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div v-if="selectedFile" class="flex flex-col gap-4">
          ...
          <div v-if="isSaving" class="w-full">
            ...
            <p class="text-sm text-gray-600 mt-2 text-center">Importing models data...</p>
          </div>
          <Button icon="pi pi-save" :label="isSaving ? 'Saving...' : 'Import Models'" severity="success"
    ...
    <div v-if="showResults && importResults" class="bg-white rounded-lg p-6 shadow-md">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold">Models Import Results</h2>
      ...
      <div v-if="importResults?.success?.valid_records > 0 && importResults?.success?.valid_data">
        ...
        <DataTable v-model:filters="filters" :value="importResults.success.valid_data" paginator :rows="10" dataKey="id"
          filterDisplay="menu"
          :globalFilterFields="['title', 'slug', 'resource_type']"
          class="th-table">
          <template #header>
            <div class="flex justify-between items-center">
              <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
