...
import { resolveMediaImage, mediaUrl } from '@/utils/mediaUrl';

const router = useRouter();
const finishStore = useFinishStore();
const { finishes, fb } = storeToRefs(finishStore);
const filters = ref({ ...new DocumentFilter() });
const toast = useToast();

const editDocument = (data: any) => {
  router.push(`/design-resource-finishes/edit/${data.design_resource_id}`);
};

const deleteDocument = async (data: any) => {
  try {
    await finishStore.deleteFinish(data.design_resource_id);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Finish deleted successfully',
      life: 3000
    });
  } catch (error) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to delete finish.',
      life: 3000
    });
  }
};
...
const renderIcon = (icon: string, action: string, id: number) => {
  return fb.value.loading[`finish.${action}.${id}`] ? 'pi pi-spinner pi-spin' : icon;
};
...
watch(finishes.value, () => { });

onMounted(() => {
  finishStore.fetchFinishes();
  console.log('finishes ', finishes.value);
});
</script>

<template>
  <div class="document-index">
    <DataTable v-model:filters="filters" :value="finishes" paginator :rows="10" dataKey="design_resource_document_id"
      filterDisplay="menu" :loading="fb.loading['finish.list']"
      :globalFilterFields="['title', 'type', 'brand', 'image_alt', 'link_text', 'action']" selectionMode="multiple"
      class="th-table">
      <template #header>
      ...
      <template #empty> No finish to display! </template>
      <template #loading> Loading finish data. Please wait. </template>
      <Column selectionMode="multiple" headerStyle="width: 3rem" />

      <Column field="image_thumb_url" header="Image" style="min-width: 12rem">
        <template #body="{ data }">
          <img :src="resolveMediaImage(data.image_thumb_url)" :alt="data.image_thumb_url"
            style="width: 50px; height: 50px; object-fit: cover; cursor: pointer;" class="shadow-2 border-round" />
        </template>
      </Column>
      ...
      <Column header="Actions" :exportable="false" style="min-width: 8rem">
        <template #body="{ data }">
          <div class="flex gap-2">
            <Button :icon="renderIcon('pi pi-pencil', 'edit', data.design_resource_id)" rounded outlined
              severity="success" class="mr-2" @click="editDocument(data)" />
            <Button :icon="renderIcon('pi pi-trash', 'delete', data.design_resource_id)" rounded outlined
              severity="danger" @click="deleteDocument(data)" />
          </div>
        </template>
      </Column>
    </DataTable>
