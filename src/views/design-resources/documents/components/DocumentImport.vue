...
const handleSave = async () => {
  ...
  try {
    ...
    const response = await designResourceImportStore.createDocumentsImport(formData);

    // Store the import results
    importResults.value = response;
    showResults.value = true;

    toast.add({ severity: 'success', summary: 'Success', detail: 'Documents data imported successfully', life: 3000 });
    selectedFile.value = null;
  } catch (error) {
    console.error('Import error:', error);
    toast.add({ severity: 'error', summary: 'Error', detail: error.message || 'Failed to import documents data', life: 3000 });
  } finally {
  ...
  <div class="flex flex-col gap-8">
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">
        Import Documents CSV data
      </h2>
      <p class="text-gray-600 mb-4">Upload a CSV file to start importing documents data</p>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div v-if="selectedFile" class="flex flex-col gap-4">
          ...
          <div v-if="isSaving" class="w-full">
            ...
            <p class="text-sm text-gray-600 mt-2 text-center">Importing documents data...</p>
          </div>
          <Button icon="pi pi-save" :label="isSaving ? 'Saving...' : 'Import Documents'" severity="success"
    ...
    <div v-if="showResults && importResults" class="bg-white rounded-lg p-6 shadow-md">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold">Documents Import Results</h2>
      ...
      <div v-if="importResults?.success?.invalid_records > 0 && importResults?.success?.invalid_data" class="mb-6">
        <h3 class="text-lg font-semibold mb-4 text-red-700">Invalid Records ({{ importResults.success.invalid_records
        }})</h3>
        <DataTable v-model:filters="filters" :value="importResults.success.invalid_data" paginator :rows="10"
      ...
      <div v-if="importResults?.success?.updated_records > 0 && importResults?.success?.updated_data" class="mb-6">
        <h3 class="text-lg font-semibold mb-4 text-orange-700">Updated Records ({{ importResults.success.updated_records
        }})
        </h3>
        <DataTable v-model:filters="filters" :value="importResults.success.updated_data" paginator :rows="10"
          ...
          <!-- title -->
          <Column field="data.title" header="Title" style="min-width: 10rem">
            <template #body="{ data }">
              {{ data.data.title }}
            </template>
          </Column>
      ...
      <div v-if="importResults?.success?.valid_records > 0 && importResults?.success?.valid_data">
        <h3 class="text-lg font-semibold mb-4 text-green-700">Valid Records ({{ importResults.success.valid_records }})
        </h3>
        <DataTable v-model:filters="filters" :value="importResults.success.valid_data" paginator :rows="10" dataKey="id"
          filterDisplay="menu"
          :globalFilterFields="['title', 'slug', 'resource_type', 'type', 'img', 'img2', 'brand', 'description', 'grade', 'link_text']"
          class="th-table">
          <template #header>
