...
import DocumentImportResponse from '@/models/DesignResourceImport';
...
const { importExport, fb } = storeToRefs(documentStore);
// console.log('import page ', importExport);

const toast = useToast();
// Length type Import related states
const selectedFileDocuments = ref<File | null>(null);
const filters = ref({ global: { value: null, matchMode: 'contains' } })
const showDocumentResults = computed(() => {
  return (
...
const handleFileSelect = (event: any) => {
  ...
  selectedFileDocuments.value = file;
};

// Handle variant import (save action)
const handleSave = async () => {
  if (!selectedFileDocuments.value) {
    toast.add({
      severity: 'error',
      summary: 'Error',
  ...
  try {
    ...
    formData.append('csv_file', selectedFileDocuments.value);
    await documentStore.importDocuments(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
    ...
    selectedFileDocuments.value = null;
  } catch (error: any) {
...
const clearDocumentResults = () => {
  importExport.value = new DocumentImportResponse();
};
...
<template>
  <div class="variant-import p-6">
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">Import document CSV data</h2>
      <p class="text-gray-600 mb-4">Upload a CSV file to start importing document data</p>
      <div class="flex flex-col gap-4">
        <FileUpload mode="basic" name="importDocumentFile" accept=".csv" :maxFileSize="1000000" @select="handleFileSelect"
          :auto="false" chooseLabel="Select CSV File" />
        <div v-if="selectedFileDocuments" class="flex flex-col gap-4">
          <div class="flex items-center gap-2 p-3 bg-gray-50 rounded">
            <i class="pi pi-file text-gray-500"></i>
            <span class="text-gray-700">{{ selectedFileDocuments.name }}</span>
          </div>
          <div v-if="fb.loading['document.import']" class="w-full">
            <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
            <p class="text-sm text-gray-600 mt-2 text-center">Importing document data...</p>
          </div>
          <Button icon="pi pi-save" :label="fb.loading['document.import'] ? 'Saving...' : 'Import document'"
            severity="success" @click="handleSave" :loading="fb.loading['document.import']"
            :disabled="fb.loading['document.import']" class="p-button-md" />
        </div>
      </div>

      <!-- Length types Import Results Section -->
      <div v-if="showDocumentResults" class="bg-white rounded-lg p-6 shadow-md mt-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="100" dataKey="row"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                ...
                <InputText v-model="filters['global'].value" placeholder="Search invalid document records..." />
              </div>
            </template>
            <template #empty>No invalid document records found.</template>
        ...
        <div v-if="importExport.duplicated_records > 0 && importExport.duplicated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.duplicated_data" paginator :rows="200" dataKey="row"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search duplicate documents records..." />
              </div>
            </template>
            <template #empty>No invalid documents records found.</template>
            <Column field="row" header="Row" style="min-width: 6rem">
        ...
        <div v-if="importExport.updated_records > 0 && importExport.updated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.updated_data" paginator :rows="100"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search updated document records..." />
              </div>
            </template>
            <template #empty>No updated document records found.</template>
        ...
        <div v-if="importExport.valid_records > 0 && importExport.valid_data">
          ...
          <DataTable v-model:filters="filters" :value="importExport.valid_data" paginator :rows="100" dataKey="length_type_id"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search valid document records..." />
              </div>
            </template>
