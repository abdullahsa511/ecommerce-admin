...
function onDragStart(e, index) {
    console.log('onDragStart', e, index);
    isDragging = true;
...
function onDragOver(e) {
    ...
    leftPx = Math.max(0, Math.min(leftPx, rect.width));
    topPx = Math.max(0, Math.min(topPx, rect.height));

    point.leftPx = leftPx;
    point.topPx = topPx;
    point._usePx = true;

    point.positionPercent.left = (leftPx / rect.width) * 100;
    point.positionPercent.top = (topPx / rect.height) * 100;
...
function addWayPoint() {
    ...
    // After DOM updates, initialize percentage position for the new point if possible
    nextTick(() => {
        const idx = pointsState.value.length - 1;
        initializePosition(idx);
        updateLineWidth(idx);
    });
...
<template>
    <div id="banner-wrapper" ref="bannerWrapper" style="position: relative; width: 100%; max-width: 100%">
        ...
        <div class="th-hero-wrapper th-hero gr-bg4 background-image" ref="bannerImg" id="wey-point-layout"
            @load="onImageLoad" :style="{ backgroundImage: 'url(' + imgSrc + ')' }"
           >

            <div v-for="(point, index) in pointsState" :key="point.id" class="way-point"
                :ref="(el) => setWayPointRef(el, index)" draggable="true"
                :style="{ left: point.positionPercent.left !== null ? point.positionPercent.left + '%' : point.leftPx + 'px', top: point.positionPercent.top !== null ? point.positionPercent.top + '%' : point.topPx + 'px' }" @mousedown="onMouseDown"
