...
onMounted(async () => {
    try {
        ...
        const fetchedReturnReason = await returnReasonStore.findReturnReasonById(returnReasonId);
        returnReason.value = new ReturnReason(fetchedReturnReason);
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to load return reason details. Please try again.',
            life: 3000
        });
...
const updateReturnReason = async () => {
    try {
        await returnReasonStore.updateReturnReason(returnReasonId, returnReason.value);
        toast.add({
            severity: 'success',
            summary: 'Success',
            detail: 'Return reason updated successfully',
            life: 3000
        });
        goToReturnReasonList();
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to update return reason. Please try again.',
            life: 3000
        });
    }
};
                ...
                <div class="grid grid-cols-12 gap-2">
                    ...
                    <div class="col-span-12 md:col-span-10">
                        <IconField>
                            ...
                            placeholder="loading..."
                            id="name" />
                            <InputIcon v-if="fb.loading.returnReason" class="pi pi-spin pi-spinner" />
                        </IconField>
                    </div>
                </div>
