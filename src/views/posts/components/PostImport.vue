...
const handleSavePosts = async () => {
    ...
    try {
        ...
        const response = await postStore.createPostImport(formData);
...
<template>
    <div class="flex flex-col gap-8">
        ...
        <div class="bg-white rounded-lg p-6 shadow-md">
            <h2 class="text-xl font-semibold mb-2">Import Posts CSV data</h2>
            <p class="text-gray-600 mb-4">Upload a CSV file to start importing posts data</p>
                ...
                <div v-if="selectedFilePosts" class="flex flex-col gap-4">
                    ...
                    <div v-if="isSavingPosts" class="w-full">
                        <ProgressBar 
                            ...
                            class="w-full"
                        />
                        <p class="text-sm text-gray-600 mt-2 text-center">Importing posts data...</p>
                    </div>

                    <Button 
                        icon="pi pi-save"
                        :label="isSavingPosts ? 'Saving...' : 'Import Posts'"
                        severity="success"
                        @click="handleSavePosts"
        ...
        <div class="bg-white rounded-lg p-6 shadow-md">
            <h2 class="text-xl font-semibold mb-2">Import Post Images CSV data</h2>
            <p class="text-gray-600 mb-4">Upload a CSV file to start importing post image data</p>
        ...
        <div v-if="showPostsResults && postsImportResults" class="bg-white rounded-lg p-6 shadow-md">
            ...
            <div v-if="postsImportResults?.success?.invalid_records > 0 && postsImportResults?.success?.invalid_data" class="mb-6">
                ...
                <DataTable
                    ...
                    dataKey="row"
                    filterDisplay="menu"
                    :globalFilterFields="['data.site_id', 'data.status_id', 'data.admin_id', 'data.name', 'data.slug']"
                    class="th-table"
                >
                    <template #header>
                    ...
                    <Column field="data.status_id" header="Status" style="min-width: 8rem">
                        <template #body="{ data }">
                            {{ data.data.status_id }}
                        </template>
                    </Column>
            ...
            <div v-if="postsImportResults?.success?.updated_records > 0 && postsImportResults?.success?.updated_data" class="mb-6">
                ...
                <DataTable
                    ...
                    dataKey="post_id"
                    filterDisplay="menu"
                    :globalFilterFields="['post_id', 'site_id', 'status_id', 'admin_id', 'name', 'slug']"
                    class="th-table"
                >
                    <template #header>
                        <div class="flex justify-between items-center">
                    ...
                    <Column field="status_id" header="Status" style="min-width: 8rem" />
