...
const { importExport, fb } = storeToRefs(weightTypeStore);
// console.log('import page ', importExport);

const toast = useToast();
// Weight type Import related states
const selectedFileWeightTypes = ref<File | null>(null);
const filters = ref({ global: { value: null, matchMode: 'contains' } })
const showWeightTypeResults = computed(() => {
  return (
...
const handleFileSelect = (event: any) => {
  ...
  selectedFileWeightTypes.value = file;
};

// Handle variant import (save action)
const handleSave = async () => {
  if (!selectedFileWeightTypes.value) {
    toast.add({
      severity: 'error',
      summary: 'Error',
  ...
  try {
    ...
    formData.append('csv_file', selectedFileWeightTypes.value);
    await weightTypeStore.importWeightTypes(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Weight Type imported successfully',
      life: 3000
    });
    selectedFileWeightTypes.value = null;
  } catch (error: any) {
...
const clearWeightTypeResults = () => {
  importExport.value = new WeightTypeImportResponse();
};
...
<template>
  <div class="variant-import p-6">
    <!-- Weight Types Import Section -->
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">Import Weight Types CSV data</h2>
      <p class="text-gray-600 mb-4">Upload a CSV file to start importing weight type data</p>
      <div class="flex flex-col gap-4">
        <FileUpload mode="basic" name="importWeightTypeFile" accept=".csv" :maxFileSize="1000000" @select="handleFileSelect"
          :auto="false" chooseLabel="Select CSV File" />
        <div v-if="selectedFileWeightTypes" class="flex flex-col gap-4">
          <div class="flex items-center gap-2 p-3 bg-gray-50 rounded">
            <i class="pi pi-file text-gray-500"></i>
            <span class="text-gray-700">{{ selectedFileWeightTypes.name }}</span>
          </div>
          <div v-if="fb.loading['weightType.import']" class="w-full">
            <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
            <p class="text-sm text-gray-600 mt-2 text-center">Importing weight type data...</p>
          </div>
          <Button icon="pi pi-save" :label="fb.loading['weightType.import'] ? 'Saving...' : 'Import weight type'"
            severity="success" @click="handleSave" :loading="fb.loading['weightType.import']"
            :disabled="fb.loading['weightType.import']" class="p-button-md" />
        </div>
      </div>

      <!-- Weight types Import Results Section -->
      <div v-if="showWeightTypeResults" class="bg-white rounded-lg p-6 shadow-md mt-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-semibold">Weight types Import Results</h2>
          <Button icon="pi pi-times" label="Clear Results" severity="secondary" outlined
            @click="clearWeightTypeResults" />
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
            <div class="text-2xl font-bold text-orange-600">{{ importExport.valid_records || 0 }}</div>
            <div class="text-sm text-orange-800">Valid Records</div>
          </div>
          <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
          ...
          <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="100" dataKey="row"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                ...
                <InputText v-model="filters['global'].value" placeholder="Search invalid weight type records..." />
              </div>
            </template>
            <template #empty>No invalid weight type records found.</template>
        ...
        <div v-if="importExport.duplicated_records > 0 && importExport.duplicated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.duplicated_data" paginator :rows="200" dataKey="row"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search duplicate weight types records..." />
              </div>
            </template>
            <template #empty>No invalid weight types records found.</template>
            <Column field="row" header="Row" style="min-width: 6rem">
        ...
        <div v-if="importExport.updated_records > 0 && importExport.updated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.updated_data" paginator :rows="100"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search updated weight type records..." />
              </div>
            </template>
            <template #empty>No updated weight type records found.</template>
            <Column field="data.name" header="Name" style="min-width: 10rem">
        ...
        <div v-if="importExport.valid_records > 0 && importExport.valid_data">
          ...
          <DataTable v-model:filters="filters" :value="importExport.valid_data" paginator :rows="100" dataKey="weight_type_id"
            filterDisplay="menu" :globalFilterFields="['name', 'unit']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search valid weight type records..." />
              </div>
            </template>
            <template #empty>No valid weight type records found.</template>
