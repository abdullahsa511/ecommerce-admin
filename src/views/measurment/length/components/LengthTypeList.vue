...
<template>
    <div class="lengthType-index">
        <DataTable v-model:filters="filters" :value="lengthTypes" paginator :rows="10" dataKey="length_type_id"
            ...
            :globalFilterFields="['name', 'unit', 'value']" selectionMode="multiple" class="th-table">
            <template #header>
            ...
            <Column field="name" header="Name" style="min-width: 12rem">
                ...
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter" placeholder="Keyword Search" />
                </template>
            </Column>
            ...
            <Column header="Actions" :exportable="false" style="min-width: 8rem">
                <template #body="{ data }">
                    <div class="flex gap-2">
                        ...
                        <Button 
                        :icon="fb.loading['lengthType.delete.'+data.length_type_id] ? 'pi pi-spinner pi-spin' : 'pi pi-trash'"
                        :disabled="fb.loading['lengthType.delete.'+data.length_type_id]"
                        rounded outlined 
                        severity="danger" 
