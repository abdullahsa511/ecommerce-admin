...
const { importExport, fb } = storeToRefs(orderStore);
// console.log('import page ', importExport);

const toast = useToast();
// Order Import related states
const selectedFileOrders = ref<File | null>(null);
const filters = ref({ global: { value: null, matchMode: 'contains' } })
const showOrderResults = computed(() => {
  return (
...
const handleSave = async () => {
  ...
  try {
    ...
    formData.append('csv_file', selectedFileOrders.value);
    await orderStore.importOrders(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Order imported successfully',
      life: 3000
    });
    selectedFileOrders.value = null;
  } catch (error: any) {
...
const clearOrderResults = () => {
  importExport.value = new OrderImportResponse();
};
...
<template>
  <div class="variant-import p-6">
    <!-- Orders Import Section -->
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">Import Orders CSV data</h2>
      <p class="text-gray-600 mb-4">Upload a CSV file to start importing order data</p>
      <div class="flex flex-col gap-4">
        <FileUpload mode="basic" name="importOrderFile" accept=".csv" :maxFileSize="1000000" @select="handleFileSelect"
          :auto="false" chooseLabel="Select CSV File" />
        <div v-if="selectedFileOrders" class="flex flex-col gap-4">
          <div class="flex items-center gap-2 p-3 bg-gray-50 rounded">
            <i class="pi pi-file text-gray-500"></i>
            <span class="text-gray-700">{{ selectedFileOrders.name }}</span>
          </div>
          <div v-if="fb.loading['order.import']" class="w-full">
            <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
            <p class="text-sm text-gray-600 mt-2 text-center">Importing order data...</p>
          </div>
          <Button icon="pi pi-save" :label="fb.loading['order.import'] ? 'Saving...' : 'Import order'"
            severity="success" @click="handleSave" :loading="fb.loading['order.import']"
            :disabled="fb.loading['order.import']" class="p-button-md" />
        </div>
      </div>

      <!-- Orders Import Results Section -->
      <div v-if="showOrderResults" class="bg-white rounded-lg p-6 shadow-md mt-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-semibold">Orders Import Results</h2>
          <Button icon="pi pi-times" label="Clear Results" severity="secondary" outlined
            @click="clearOrderResults" />
        </div>
          ...
          <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="100" dataKey="row"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                ...
                <InputText v-model="filters['global'].value" placeholder="Search invalid order records..." />
              </div>
            </template>
            <template #empty>No invalid order records found.</template>
        ...
        <div v-if="importExport.duplicated_records > 0 && importExport.duplicated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.duplicated_data" paginator :rows="200" dataKey="row"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search duplicate orders records..." />
              </div>
            </template>
            <template #empty>No invalid orders records found.</template>
            <Column field="row" header="Row" style="min-width: 6rem">
        ...
        <div v-if="importExport.updated_records > 0 && importExport.updated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.updated_data" paginator :rows="100"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search updated order records..." />
              </div>
            </template>
            <template #empty>No updated order records found.</template>
        ...
        <div v-if="importExport.valid_records > 0 && importExport.valid_data">
          ...
          <DataTable v-model:filters="filters" :value="importExport.valid_data" paginator :rows="100" dataKey="length_type_id"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search valid order records..." />
              </div>
            </template>
