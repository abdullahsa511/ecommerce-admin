<script setup lang="ts">
    ...
    type FocusableLocalField = 'custom_date_format' | 'custom_time_format';
    
    const customDateFormatField = ref<HTMLElement | null>(null);
    const customTimeFormatField = ref<HTMLElement | null>(null);
    
    const focusLocalField = async (field: FocusableLocalField) => {
      await nextTick();
      const target =
        field === 'custom_date_format' ? customDateFormatField.value : customTimeFormatField.value;
    ...
    const clearFocusQuery = () => {
      ...
      if (!('focusField' in currentQuery) && !('focusStamp' in currentQuery)) {
        return;
      }
    ...
    watch(
      () => [route.query.focusField, route.query.focusStamp],
      ([field]) => {
        const value = Array.isArray(field) ? field[0] : field;
        if (value === 'custom_date_format' || value === 'custom_time_format') {
          focusLocalField(value as FocusableLocalField).finally(() => clearFocusQuery());
        }
      },
      { immediate: true }
    );
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="grid grid-cols-12 gap-2">
                ...
                <div class="col-span-12 md:col-span-10">
                    <InputText v-model="site.localSettings.address" class="w-full text-lg" 
                    :class="{'p-invalid': fb?.errors?.['site.update.' + props.site.site_id]?.address || fb?.errors?.['site.create']?.address}" 
                    />
                        <Message v-if="props.fb?.errors?.['site.update.' + props.site.site_id]?.address 
                        || props.fb?.errors?.['site.create']?.address" severity="error" size="small" variant="title">
                            <span v-for="error in props.fb?.errors?.['site.update.' + props.site.site_id]?.address 
                            || props.fb?.errors?.['site.create']?.address" :key="error" class="flex flex-col">
                                {{ error }}
                            </span>
                        </Message>
                </div>
            ...
            <div class="mt-8">
                ...
                <div class="grid grid-cols-12 gap-2 mb-6">
                    ...
                    <div class="col-span-12 md:col-span-10">
                        ...
                        <div v-if="site.localSettings.date_format === 'custom'" class="mt-2" ref="customDateFormatField">
                    ...
                    <div class="grid grid-cols-12 gap-2">
                        ...
                        <div class="col-span-12 md:col-span-10">
                            ...
                            <div v-if="site.localSettings.time_format === 'custom'" class="mt-2" ref="customTimeFormatField">
