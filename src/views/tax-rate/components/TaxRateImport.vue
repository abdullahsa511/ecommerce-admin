...
const { importExport, fb } = storeToRefs(taxRateStore);
...
const handleSave = async () => {
  ...
  try {
    ...
    await taxRateStore.taxRateImport(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
...
<template>
  ...
  <div v-if="showResults" class="bg-white rounded-lg p-6 shadow-md">
    ...
    <div v-if="importExport.invalid_records > 0 && importExport.invalid_data" class="mb-6">
      ...
      <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="200" dataKey="row"
        filterDisplay="menu" :globalFilterFields="['name', 'rate', 'type']" class="th-table">
        <template #header>
        ...
        <Column field="rate" header="Rate" style="min-width: 10rem">
          <template #body="{ data }">{{ data.data?.rate || data.rate || '' }}</template>
        </Column>
    ...
    <div v-if="importExport.updated_records > 0 && importExport.updated_data">
      ...
      <DataTable v-model:filters="filters" :value="importExport.updated_data" paginator :rows="200" dataKey="row"
        ...
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search updated tax rates records..." />
          </div>
        </template>
        <template #empty>No updated tax rates records found.</template>
        <Column field="name" header="Name" style="min-width: 10rem">
          <template #body="{ data }">{{ data.data?.name || data.name || '' }}</template>
        </Column>
        <Column field="rate" header="Rate" style="min-width: 10rem">
  ...
  <div v-if="fb.errors['taxRate.import']" class="text-red-600 text-sm pt-5">
    <div v-html="fb.renderErrors('taxRate.import')"></div>
  </div>
</template>
