...
const {fb} = storeToRefs(paymentStatusStore);

const paymentStatusId = route.params.id;

const isLoading = computed(() => {
    return fb.value.loading === true;
});

const goToPaymentStatusList = () => {
    router.push('/configuration/settings/payment-statuses');
};

onMounted(async () => {
    try {
        languageStore.fetchLanguages();
        const fetchedPaymentStatus = await paymentStatusStore.findPaymentStatusById(paymentStatusId);
        paymentStatus.value = new PaymentStatus(fetchedPaymentStatus);
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
...
const updatePaymentStatus = async () => {
    try {
        await paymentStatusStore.updatePaymentStatus(paymentStatusId, paymentStatus.value);
        toast.add({
            severity: 'success',
            summary: 'Success',
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to update payment status. Please try again.',
            life: 3000
        });
    }
};
...
<template>
    <div class="flex justify-end gap-2 mb-6">
        <Button 
            ...
            label="Payment Status List"
            severity="secondary"
            @click="goToPaymentStatusList"
            class="p-button-md"
        />
        <Button 
            icon="pi pi-save"
                ...
                <div class="grid grid-cols-12 gap-2">
                    ...
                    <div class="col-span-12 md:col-span-10">
                        <IconField>
                            ...
                            placeholder="loading..."
                            id="name" />
                            <InputIcon v-if="fb.loading.paymentStatus" class="pi pi-spin pi-spinner" />
                        </IconField>
                    </div>
                </div>
