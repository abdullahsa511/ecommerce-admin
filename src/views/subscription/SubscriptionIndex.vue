...
const { subscription, fb } = storeToRefs(subscriptionStore);
...
const items = ref([
  { route: `/ecommerce/sales/subscriptions/list`, label: 'Subscription List', icon: 'pi pi-home' },
  { route: `/ecommerce/sales/subscriptions/add`, label: 'Add', icon: 'pi pi-link' },
  { route: `/ecommerce/sales/subscriptions/import`, label: 'Import', icon: 'pi pi-link' },
]);
...
const refreshTabs = () => {
  ...
  if (routeId) {
    const editRoute = `/ecommerce/sales/subscriptions/edit/${routeId}`;
...
const renderIcon = (icon: string) => {
    return fb.value.loading[`subscription.update.${subscription.value.subscription_id}`] ? 'pi pi-spinner pi-spin' : icon;
}
...
<template>
  <div class="subscription card relative">
    ...
    <div class="sticky top-0 z-50 bg-white p-2">
            <Button v-if="showSaveButton" :label="saveLabel" :icon="renderIcon('pi pi-save')" class="p-button-md"
                @click="handleSave" :disabled="fb.loading['subscription.update.'+subscription.subscription_id]"/>
        </div>
