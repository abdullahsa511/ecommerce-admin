...
const { importExport, fb } = storeToRefs(taxTypeStore);
...
const handleSave = async () => {
  ...
  try {
    ...
    await taxTypeStore.importTaxTypes(formData);
    console.log('importExport after import', importExport.value);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Tax type imported successfully',
      life: 3000,
    });
...
// Clear results
const clearResults = () => {
  importExport.value = new TaxTypeImportResponse();
};
// Clear filters
const clearFilter = () => {
...
<template>
  <!-- Tax type Import Section -->
  <div class="bg-white rounded-lg p-6 shadow-md">
    <h2 class="text-xl font-semibold mb-2">Import Tax type CSV data</h2>
    ...
    <div class="flex flex-col gap-4">
      ...
      <div v-if="selectedFile" class="flex flex-col gap-4">
        ...
        <div v-if="fb.loading['taxType.import']" class="w-full">
          ...
          <p class="text-sm text-gray-600 mt-2 text-center">Importing Tax type data...</p>
        </div>
        <Button
          icon="pi pi-save"
          :label="fb.loading['taxType.import'] ? 'Saving...' : 'Import Tax type'"
          severity="success"
          @click="handleSave"
  ...
  <!-- Tax type Import Results Section -->
  <div v-if="showResults" class="bg-white rounded-lg p-6 shadow-md">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-semibold">Tax type Import Results</h2>
      ...
      >
        <template #header>
          <div class="flex justify-between items-center">
            ...
            <InputText v-model="filters['global'].value" placeholder="Search invalid tax type records..." />
          </div>
        </template>
        <template #empty>No invalid tax type records found.</template>
        ...
        <Column field="data.content" header="Content" style="min-width: 10rem">
          <template #body="{ data }">{{ (data.data && data.data.content) ? data.data.content : data.content }}</template>
        </Column>
    ...
    <div v-if="importExport.duplicated_records > 0 && importExport.duplicated_data" class="mb-6">
      ...
      <DataTable
        ...
        dataKey="row"
        filterDisplay="menu"
        :globalFilterFields="['name', 'content']"
        class="th-table"
      >
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search duplicated tax type records..." />
          </div>
        </template>
        <template #empty>No duplicated tax type records found.</template>
        <Column field="data.name" header="Name" style="min-width: 10rem">
          <template #body="{ data }">{{ (data.data && data.data.name) ? data.data.name : data.name }}</template>
        </Column>
        <Column field="data.content" header="Content" style="min-width: 10rem">
    ...
    <div v-if="importExport.valid_records > 0 && importExport.valid_data">
      ...
      >
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search valid tax type content records..." />
          </div>
        </template>
        <template #empty>No valid tax type content records found.</template>
