...
const editTaxType = (taxType) => {
  router.push(`/configuration/settings/tax-types/edit/${taxType.tax_type_id}`);
};
...
<template>
  <div>
    <DataTable
      ...
      dataKey="tax_type_id"
      filterDisplay="menu"
      :loading="fb.loading['taxType.list']"
      ...
      <Column header="Actions" :exportable="false" style="min-width: 8rem">
        <template #body="{ data }">
          <div class="flex gap-2">
            <Button 
              icon="pi pi-pencil" 
              :disabled="fb.loading['taxType.edit.' + data.tax_type_id]"
              :loading="fb.loading['taxType.edit.' + data.tax_type_id]"
              :icon="fb.loading['taxType.edit.' + data.tax_type_id] ? 'pi pi-spinner pi-spin' : 'pi pi-pencil'"
              rounded 
              outlined 
              severity="success" 
              class="mr-2"
            ...
            <Button 
              icon="pi pi-trash" 
              :disabled="fb.loading['taxType.delete.' + data.tax_type_id]"
              :loading="fb.loading['taxType.delete.' + data.tax_type_id]"
              :icon="fb.loading['taxType.delete.' + data.tax_type_id] ? 'pi pi-spinner pi-spin' : 'pi pi-trash'"
              rounded 
              outlined 
              severity="danger"
