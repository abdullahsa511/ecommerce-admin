      ...
      <div v-if="showResults && importResults" class="bg-white rounded-lg p-6 shadow-md mt-6">
        ...
        <div v-if="importResults.invalid_records > 0 && importResults.invalid_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importResults.invalid_data" paginator :rows="10"
            ...
            :globalFilterFields="['data.variant_code', 'data.language_code', 'data.value']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search invalid variants records..." />
              </div>
            </template>
            ...
            <Column field="data.web_product_variant" header="Varaiant Name" style="min-width: 15rem">
              <template #body="{ data }">
                {{ data.data.web_product_variant }}
              </template>
            </Column>
            <Column field="data.web_product_range" header="Product Name" style="min-width: 10rem">
              <template #body="{ data }">
        ...
        <div v-if="importResults.updated_records > 0 && importResults.updated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importResults.updated_data" paginator :rows="10"
            ...
            :globalFilterFields="['data.web_product_variant', 'data.web_product_range', 'data.active_status', 'data.created_at']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search updated variants records..." />
              </div>
            </template>
            <template #empty>No updated variants records found.</template>
            <Column field="data.web_product_variant" header="Varaiant Name" style="min-width: 15rem">
              <template #body="{ data }">
                {{ data.data.web_product_variant }}
              </template>
        ...
        <div v-if="importResults.valid_records > 0 && importResults.valid_data">
          <h3 class="text-lg font-semibold mb-4 text-green-700">
            Valid Records ({{ importResults.valid_records }})
          </h3>
          <DataTable v-model:filters="filters" :value="importResults.valid_data" paginator :rows="10"
            dataKey="code" filterDisplay="menu" :globalFilterFields="['data.web_product_variant', 'data.web_product_range', 'data.active_status', 'data.created_at']"
            class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search valid variant content records..." />
              </div>
            </template>
