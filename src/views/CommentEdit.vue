...
const loadComment = async () => {
    try {
        loading.value = true;
        const comment = await commentStore.fetchCommentById(commentId);
        formData.value = {
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to load comment. Please try again later.',
            life: 3000
        });
...
const saveComment = async () => {
    try {
        loading.value = true;
        await commentStore.updateComment(commentId, formData.value);
        toast.add({
            severity: 'success',
            summary: 'Success',
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to update comment. Please try again later.',
            life: 3000
        });
    } finally {
        loading.value = false;
