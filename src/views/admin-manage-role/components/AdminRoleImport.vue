...
const { importExport, fb } = storeToRefs(roleStore);
...
const handleSave = async () => {
  ...
  try {
    ...
    await roleStore.roleImport(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
...
<template>
  ...
  <div v-if="showResults" class="bg-white rounded-lg p-6 shadow-md">
    ...
    <div v-if="importExport.invalid_records > 0 && importExport.invalid_data" class="mb-6">
      ...
      <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="200" dataKey="row"
        filterDisplay="menu" :globalFilterFields="['name', 'display_name', 'permissions']" class="th-table">
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search invalid admin roles records..." />
          </div>
        </template>
        <template #empty>No invalid admin roles records found.</template>
        ...
        <Column field="permissions" header="Permissions" style="min-width: 10rem">
          <template #body="{ data }">{{ data.data?.permissions || data.permissions || '' }}</template>
        </Column>
    ...
    <div v-if="importExport.valid_records > 0 && importExport.valid_data">
      ...
      <DataTable v-model:filters="filters" :value="importExport.valid_data" paginator :rows="200" dataKey="attribute_id"
        filterDisplay="menu"
        :globalFilterFields="['name', 'display_name', 'permissions']"
        class="th-table">
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
        ...
        <Column field="permissions" header="Permissions" style="min-width: 10rem">
          <template #body="{ data }">{{ data.permissions }}</template>
        </Column>
      </DataTable>
    </div>
  ...
  <div v-if="fb.errors['role.import']" class="text-red-600 text-sm pt-5">
    <div v-html="fb.renderErrors('role.import')"></div>
  </div>
</template>
