...
onMounted(async () => {
    try {
        const fetchedRole = await roleStore.fetchRoleById(roleId);
        console.log('Fetched Role:', fetchedRole);
        newRole.value = new Role(fetchedRole);
        console.log('Assigned Role:', newRole.value);
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to load role details. Please try again.',
            life: 3000
        });
        goToRoleList();
    }
...
const updateRole = async () => {
    try {
        await roleStore.updateRole(roleId, newRole.value);
        toast.add({
            severity: 'success',
            summary: 'Success',
