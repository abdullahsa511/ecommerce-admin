...
const isLoading = computed(() => {
    return fb.value.loading?.productType === true;
});
...
const editProductType = (data) => {
  router.push(`/configuration/settings/edit-product-type/${data.product_type_id}`);
};
...
<template>
  <div>
    ...
    >
      ...
      <Column header="Name" filterField="productTypeNames" :showFilterMatchModes="false" :filterMenuStyle="{ width: '14rem' }" style="min-width: 14rem">
        <template #body="{ data }">
          <a 
        ...
        <template #filter="{ filterModel }">
          <MultiSelect v-model="filterModel.value" :options="productTypeNames" placeholder="Any">
