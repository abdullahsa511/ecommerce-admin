<script setup lang="ts">
  ...
  const { importExport, fb } = storeToRefs(productTypeStore);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await productTypeStore.productTypeImport(formData);
      toast.add({
        severity: 'success',
        summary: 'Success',
  ...
  // Clear results
  const clearResults = () => {
    importExport.value = new ProductTypeImportResponse();
  };
  // Clear filters
  const clearFilter = () => {
  ...
  <template>
    ...
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">Import Product Type CSV data</h2>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div v-if="selectedFile" class="flex flex-col gap-4">
          ...
          <div v-if="fb.loading['productType.import']" class="w-full">
          ...
          <Button icon="pi pi-save" :label="fb.loading['productType.import'] ? 'Importing...' : 'Import '" severity="success"
            @click="handleSave" :loading="fb.loading['productType.import']" :disabled="fb.loading['productType.import']"
            class="p-button-md" />
        </div>
      </div>
    </div>
    ...
    <div v-if="fb.errors['productType.import']" class="text-red-600 text-sm pt-5">
      <div v-html="fb.errors['productType.import']"></div>
    </div>
  </template>
