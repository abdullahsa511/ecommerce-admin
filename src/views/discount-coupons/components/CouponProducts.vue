...
import { Coupon, ICouponProductEdit } from '@/models/Coupon.ts';
...
// Computed property to handle both edit mode (couponProducts) and create mode (items)
const tableItems = computed(() => {
  // If we have couponProducts (edit mode), transform them to the expected format
  if (props.coupon?.couponProducts && props.coupon.couponProducts.length > 0) {
    console.log('Edit mode detected - using couponProducts:', props.coupon.couponProducts);
    return props.coupon.couponProducts.map((product: ICouponProductEdit) => ({
      coupon_id: props.coupon.coupon_id,
      product_id: product.product_id,
      item_name: product.name,
      item_description: product.description || '',
      item_quantity: 1,
      item_unit_price: product.price || 0,
      item_total: product.price || 0
    }));
  }
  // Otherwise use the regular items array (create mode)
  console.log('Create mode detected - using items:', props.coupon?.items);
  return props.coupon?.items || [];
});

const searchProducts = (e: any) => {
  const query = e.target.value?.toLowerCase();
  if (query?.length > 0) {
    couponStore.getProductList(query).then(results => {
...
function selectItem(item: VariantItem) {
  // Check if item already exists in the current items
  const currentItems = tableItems.value;
  if (!currentItems.find((i: any) => i.item_name === item.name)) {
    ...
    // If we're in edit mode (couponProducts exists), add to couponProducts
    if (newCoupon.couponProducts) {
      newCoupon.couponProducts.push({
        coupon_id: props.coupon.coupon_id || 0,
        name: item.name,
        price: item.price || 0,
        product_id: item.product_id,
      ...
      // Create mode - add to items
      if (!newCoupon.items) {
        newCoupon.items = [];
      }
      newCoupon.items.push(newCouponItem);
    }
    
    emit('update:coupon', newCoupon);
  }
  search.value = '';
...
const updateCouponItem = (e: any) => {
  const newCoupon = { ...props.coupon };
  const currentItems = tableItems.value;
  const itemIndex = currentItems.findIndex((i: any) => i === e.data);
  
  if (itemIndex !== -1) {
    const updatedItem = { ...e.data };
    updatedItem.item_total = (updatedItem.item_quantity || 0) * (updatedItem.item_unit_price || 0);
    
    // Update in the appropriate array based on mode
    if (newCoupon.couponProducts) {
      // Edit mode - update couponProducts
      newCoupon.couponProducts[itemIndex] = {
        coupon_id: props.coupon.coupon_id || 0,
        name: updatedItem.item_name,
        price: updatedItem.item_unit_price,
        product_id: updatedItem.product_id,
        description: updatedItem.item_description
      };
    } else {
      // Create mode - update items
      if (!newCoupon.items) {
        newCoupon.items = [];
      }
      newCoupon.items[itemIndex] = updatedItem;
    }
...
function removeItem(index: number) {
  ...
  // Remove from the appropriate array based on mode
  if (newCoupon.couponProducts) {
    // Edit mode - remove from couponProducts
    newCoupon.couponProducts.splice(index, 1);
  } else {
    // Create mode - remove from items
    if (newCoupon.items) {
      newCoupon.items.splice(index, 1);
    }
  }
...
const sectionTotal = computed(() =>
  tableItems.value.reduce((sum, item) => sum + ((item.item_quantity || 0) * (item.item_unit_price || 0)), 0)
);

// Computed property to detect current mode
const isEditMode = computed(() => {
  return !!(props.coupon?.couponProducts && props.coupon.couponProducts.length > 0);
});
...
function updateQuantity(item: any, newQty: number) {
  if (newQty > 0) {
    const newCoupon = { ...props.coupon };
    const currentItems = tableItems.value;
    const itemIndex = currentItems.findIndex((i: any) => i === item);
...
<template>
  <div class="quote-table-wrapper">
    ...
    <DataTable
      ...
      editMode="row"
      @row-edit-save="updateCouponItem"
      class="quote-table"
    >
      ...
      <Column header="Item Total">
        <template #body="slotProps">
          <td class="item-total">${{ ((slotProps.data.item_quantity*1 || 0) * (slotProps.data.item_unit_price*1 || 0)).toFixed(2) }}</td>
        </template>
      </Column>
