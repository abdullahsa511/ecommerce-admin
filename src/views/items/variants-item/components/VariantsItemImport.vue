<script setup lang="ts">
import { ref, onMounted, computed } from 'vue';
import InputText from 'primevue/inputtext';
import Button from 'primevue/button';
import DataTable from 'primevue/datatable';
import Column from 'primevue/column';
import { useToast } from 'primevue/usetoast';
import FileUpload from 'primevue/fileupload';
import ProgressBar from 'primevue/progressbar';
import { storeToRefs } from 'pinia';
import { useVariantsItemStore } from '@/stores/variantsItem';
import VariantItemImportResponse from '@/models/VariantItemImport';

//===========================================================
// Define Props
//===========================================================
// const props = defineProps({});

//===========================================================
// Declare Emits
//===========================================================
const emit = defineEmits(['editTabUnmunted', 'editTabMounted', 'saved']);

//===========================================================
// Define Local Variables
//===========================================================
const variantsItemStore = useVariantsItemStore();
const { importExport, fb } = storeToRefs(variantsItemStore);
// console.log('import page ', importExport);

const toast = useToast();
// Variants item Import related states
const selectedFile = ref<File | null>(null);
const filters = ref({ global: { value: null, matchMode: 'contains' } })
const showResults = computed(() => {
  return (
    importExport.value.invalid_records > 0 ||
    importExport.value.updated_records > 0 ||
    importExport.value.valid_records > 0
  );
});

//===========================================================
// Define Local Methods
//===========================================================
// Handle file selection for variant import
const handleFileSelect = (event: any) => {
  const file = event.files[0];
  if (!file) return;
  if (!file.name.endsWith('.csv')) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Please select a CSV file',
      life: 3000
    });
    return;
  }
  selectedFile.value = file;
};

// Handle variant import (save action)
const handleSave = async () => {
  if (!selectedFile.value) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Please select a file first',
      life: 3000
    });
    return;
  }
  try {
    const formData = new FormData();
    formData.append('csv_file', selectedFile.value);
    await variantsItemStore.importVariantsItems(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Variants Item imported successfully',
      life: 3000
    });
    selectedFile.value = null;
  } catch (error: any) {
    console.error('Save error:', error);
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: error.message || 'Failed to save file',
      life: 3000
    });
  } finally {
    fb.value.loading['variantItem.import'] = false;
  }
};

// Clear variant import results
const clearResults = () => {
  importExport.value = new VariantItemImportResponse();
};

// Clear filters
const clearFilter = () => {
  filters.value = { global: { value: null, matchMode: 'contains' } };
};

const getProductName = (productId: number | string) => {
  const map = importExport.value?.mapping_data?.productCodeToIdMap || {};
  for (const [name, id] of Object.entries(map)) {
    if (id === productId || String(id) === String(productId)) return name;
  }
  return productId;
};

const getVariantName = (variantId: number | string) => {
  const map = importExport.value?.mapping_data?.productVariantNameToIdMap || {};
  for (const [name, id] of Object.entries(map)) {
    if (id === variantId || String(id) === String(variantId)) return name;
  }
  return variantId;
};
const getItemName = (itemId: number | string) => {
  const map = importExport.value?.mapping_data?.itemCodeToIdMap || {};
  for (const [name, id] of Object.entries(map)) {
    if (id === itemId || String(id) === String(itemId)) return name;
  }
  return itemId;
};


//===========================================================
// Lifecycle Hooks
//===========================================================
onMounted(() => {
  console.log('import page', importExport);

  // Any initialization logic here (if needed)
});
</script>

<template>
  <div class="variant-import p-6">
    <!-- Variants Items Import Section -->
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">Import Variants Items CSV data</h2>
      <p class="text-gray-600 mb-4">Upload a CSV file to start importing variants item data</p>
      <div class="flex flex-col gap-4">
        <FileUpload mode="basic" name="importVariantsItemFile" accept=".csv" :maxFileSize="800000000"
          @select="handleFileSelect" :auto="false" chooseLabel="Select CSV File" />
        <div v-if="selectedFile" class="flex flex-col gap-4">
          <div class="flex items-center gap-2 p-3 bg-gray-50 rounded">
            <i class="pi pi-file text-gray-500"></i>
            <span class="text-gray-700">{{ selectedFile.name }}</span>
          </div>
          <div v-if="fb.loading['variantItem.import']" class="w-full">
            <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
            <p class="text-sm text-gray-600 mt-2 text-center">Importing variant item data...</p>
          </div>
          <Button icon="pi pi-save" :label="fb.loading['variantItem.import'] ? 'Importing...' : 'Import Variant Items'"
            severity="success" @click="handleSave" :loading="fb.loading['variantItem.import']"
            :disabled="fb.loading['variantItem.import']" class="p-button-md" />
        </div>
      </div>

      <!-- Variants items Import Results Section -->
      <div v-if="showResults" class="bg-white rounded-lg p-6 shadow-md mt-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-semibold">Item options Import Results</h2>
          <Button icon="pi pi-times" label="Clear Results" severity="secondary" outlined @click="clearResults" />
        </div>

        <!-- Summary Statistics -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <div class="text-2xl font-bold text-blue-600">{{ importExport.total_records || 0 }}</div>
            <div class="text-sm text-blue-800">Total Records</div>
          </div>
          <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
            <div class="text-2xl font-bold text-orange-600">{{ importExport.invalid_records || 0 }}</div>
            <div class="text-sm text-orange-800">Invalid Records</div>
          </div>
          <div class="bg-green-50 p-4 rounded-lg border border-green-200">
            <div class="text-2xl font-bold text-green-600">{{ importExport.updated_records || 0 }}</div>
            <div class="text-sm text-green-800">Records Updated</div>
          </div>
          <div class="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <div class="text-2xl font-bold text-purple-600">{{ importExport.valid_records || 0 }}</div>
            <div class="text-sm text-purple-800">Records Inserted</div>
          </div>

        </div>

        <!-- Invalid Records DataTable -->
        <div v-if="importExport.invalid_records > 0 && importExport.invalid_data" class="mb-6">
          <h3 class="text-lg font-semibold mb-4 text-red-700">Invalid Records ({{ importExport.invalid_records
            }})</h3>
          <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="20" dataKey="row"
            filterDisplay="menu"
            :globalFilterFields="['data.web_product', 'data.web_product_variant', 'data.item_code', 'data.id', 'data.item_description']"
            class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search invalid options item records..." />
              </div>
            </template>
            <template #empty>No invalid variant item records found.</template>
            <Column field="row" header="Row" style="min-width: 6rem">
              <template #body="{ data }">
                {{ data.row }}
              </template>
            </Column>
            <Column field="errors" header="Errors" style="min-width: 15rem">
              <template #body="{ data }">
                <div class="text-red-600 text-sm">
                  {{ data.errors }}
                </div>
              </template>
            </Column>
            <Column field="data.web_product" header="Product" style="min-width: 15rem">
              <template #body="{ data }">
                {{ data.data.web_product_range }}
              </template>
            </Column>
            <Column field="data.web_product_variant" header="Variant" style="min-width: 10rem">
              <template #body="{ data }">
                {{ data.data.web_product_variant }}
              </template>
            </Column>
            <Column field="data.item_code" header="Item Code" style="min-width: 10rem">
              <template #body="{ data }">
                {{ data.data.item_code }}
              </template>
            </Column>
            <Column field="data.id" header="KM Item Id" style="min-width: 10rem">
              <template #body="{ data }">
                {{ data.data.id }}
              </template>
            </Column>
            <Column field="data.item_description" header="Description" style="min-width: 10rem">
              <template #body="{ data }">
                {{ data.data.item_description }}
              </template>
            </Column>
          </DataTable>
        </div>

        <!-- Duplicate Records DataTable -->
        <div v-if="importExport.duplicated_records > 0 && importExport.duplicated_data">
          <h3 class="text-lg font-semibold mb-4 text-orange-700">
            Duplicated Records ({{ importExport.duplicated_records }})
          </h3>
          <DataTable v-model:filters="filters" :value="importExport.duplicated_data" paginator :rows="100"
            dataKey="options_item_id" filterDisplay="menu"
            :globalFilterFields="['name', 'option_id', 'product_id', 'product_variant_id', 'product_option_group_id', 'type_id', 'value', 'meta_description']"
            class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search duplicated options item records..." />
              </div>
            </template>
            <template #empty>No duplicated variant item records found.</template>
            <Column field="product_id" header="Product" style="min-width: 15rem">
              <template #body="{ data }">{{ getProductName(data.product_id) }}</template>
            </Column>
            <Column field="product_variant_id" header="Variant" style="min-width: 15rem">
              <template #body="{ data }">{{ getVariantName(data.product_variant_id) }}</template>
            </Column>
            <Column field="item_id" header="Item" style="min-width: 15rem">
              <template #body="{ data }">{{ getItemName(data.item_id) }}</template>
            </Column>
            <Column field="km_item_id" header="KM Item Id" style="min-width: 15rem">
              <template #body="{ data }">{{ data.km_item_id }}</template>
            </Column>
            <Column field="sort_order" header="Sort Order" style="min-width: 15rem">
              <template #body="{ data }">{{ data.sort_order }}</template>
            </Column>
            <Column field="active_status" header="Active Status" style="min-width: 15rem">
              <template #body="{ data }">{{ data.active_status ? 'Active' : 'Inactive' }}</template>
            </Column>
          </DataTable>
        </div>

        <!-- Updated Records DataTable -->
        <div v-if="importExport.updated_records > 0 && importExport.updated_data" class="mb-6">
          <h3 class="text-lg font-semibold mb-4 text-orange-700">
            Updated Records ({{ importExport.updated_records }})
          </h3>
          <DataTable v-model:filters="filters" :value="importExport.updated_data" paginator :rows="100"
            dataKey="option_id" filterDisplay="menu" :globalFilterFields="['name', 'unit']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search updated options item records..." />
              </div>
            </template>
            <template #empty>No updated options item records found.</template>
            <Column field="data.web_product_code" header="Product Code" style="min-width: 10rem">
              <template #body="{ data }">{{ data.web_product_code }}</template>
            </Column>
            <Column field="data.web_product_variant" header="Variant" style="min-width: 10rem">
              <template #body="{ data }">{{ data.web_product_variant }}</template>
            </Column>
            <Column field="data.item_code" header="Item Code" style="min-width: 10rem">
              <template #body="{ data }">{{ data.item_code }}</template>
            </Column>
            
            <Column field="data.item_description" header="Description" style="min-width: 10rem">
              <template #body="{ data }">{{ data.item_description }}</template>
            </Column>
          </DataTable>
        </div>

        <!-- Valid Records DataTable -->
        <div v-if="importExport.valid_records > 0 && importExport.valid_data">
          <h3 class="text-lg font-semibold mb-4 text-green-700">
            Valid Records ({{ importExport.valid_records }})
          </h3>
          <DataTable v-model:filters="filters" :value="importExport.valid_data" paginator :rows="100"
            dataKey="options_item_id" filterDisplay="menu"
            :globalFilterFields="['name', 'option_id', 'product_id', 'product_variant_id', 'product_option_group_id', 'type_id']"
            class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search valid options item records..." />
              </div>
            </template>
            <template #empty>No valid variant item records found.</template>
            <Column field="web_product_code" header="Product" style="min-width: 15rem">
              <template #body="{ data }">{{ getProductName(data.web_product_code) }}</template>
            </Column>
            <Column field="web_product_variant" header="Variant" style="min-width: 15rem">
              <template #body="{ data }">{{ getVariantName(data.web_product_variant) }}</template>
            </Column>
           
            <Column field="item_code" header="Item Code" style="min-width: 15rem" />
            <Column field="id" header="KM Item Id" style="min-width: 15rem" />
            <Column field="item_description" header="Description" style="min-width: 15rem" />
          </DataTable>
        </div>

      </div>
    </div>
  </div>
</template>

<style scoped>
.variant-import {
  padding: 2rem;
}

.p-datatable .p-datatable-thead>tr>th {
  background-color: var(--surface-100);
}

.p-datatable .p-datatable-tbody>tr>td {
  padding: 1rem;
}
</style>
