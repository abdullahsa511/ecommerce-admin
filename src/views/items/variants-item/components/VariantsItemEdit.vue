<script setup lang="ts">
  ...
  const { itemVariant } = storeToRefs(variantsItemStore);
  ...
  const searchVariants = async (event: any) => {
    ...
    if (itemVariant.value.product_id !== null && itemVariant.value.product_id !== undefined) {
      productVariantSuggestions.value = await itemStore.listItemVariants(event, itemVariant.value.product_id);
    } else {
...
const handleInsertVariant = async (variant: VariantItem) => {
  ...
  const handleSaveVariant = async (data: any) => {
    try {
      ...
      await variantsItemStore.createItemVariant(data);
      if (response) {
  ...
  const handleUpdateOptionGroup = async (group: ProductOptionGroup) => {
    ...
    if (groupIndex !== undefined && groupIndex !== null && groupIndex > -1) {
      if (itemVariant.value) {
        itemVariant.value.itemOptionGroups[groupIndex] = group;
      }
    }
  ...
  const onSelectType = (event: any) => {
    ...
    if (groupIndex !== undefined && groupIndex !== null && groupIndex > -1) {
      if (itemVariant.value && index) {
        ...
        itemVariant.value.itemOptionGroups[groupIndex].itemOptions[index] = option;
      }
    }
  }
  ...
  <div v-bind="$attrs">
    ...
    <div class="flex flex-col md:flex-row gap-2">
      ...
      <div
        ...
        <VariantForm v-if="!itemVariant.value?.product_variant_id"
          :fb="fb" v-show="showVariantCard" :productVariants="[itemVariant.value]"
          :productVariantSuggestions="productVariantSuggestions" :variant="itemVariant" :buttonLabel="'Save Variant'"
          @searchVariant="searchVariants"
          @add:variant="handleAddVariant" 
          @insert:variant="handleInsertVariant"
          @submit="handleSaveVariant" @inputVariant="inputVariant"/>

        <Variant :key="itemVariant.product_variant_id"  :variant="itemVariant"
          :productVariantSuggestions="productVariantSuggestions"
          @add:variant="handleAddVariant" 
          :showForm="!!itemVariant.selected" :fb="fb" :buttonLabel="'Update Variant'" @update:variant="handleUpdateVariant"
          @delete:variant="handleDeleteVariant" @edit:variant="toggleVariant()"
          @searchVariant="searchVariants"
          />
      </div>
      ...
      <div ref="optionGroupsHeadingRef"
        ...
        <OptionGroupForm v-show="showOptionGroupCard" :optionGroupData="itemVariant?.itemOptionGroups" :optionGroup="optionGroup"
