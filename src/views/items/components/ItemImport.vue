...
const { importExport, dimensionImportExport, fb } = storeToRefs(ItemStore);

const toast = useToast();
//  item Import related states
const selectedFileItems = ref<File | null>(null);
const selectedFileDimensions = ref<File | null>(null);
...
// =========================================================
// Handle file selection for dimension import
const handleDimensionFileSelect = (event: any) => {
  const file = event.files[0];
  if (!file) return;
  if (!file.name.endsWith('.csv')) {
    toast.add({
  ...
  selectedFileDimensions.value = file;
};

// Handle dimension import (save action)
const handleDimensionSave = async () => {
  if (!selectedFileDimensions.value) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Please select a file first',
  ...
  try {
    const formData = new FormData();
    formData.append('csv_file', selectedFileDimensions.value);
    await ItemStore.importDimensions(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Dimensions imported successfully',
      life: 3000
    });
    selectedFileDimensions.value = null;
  } catch (error: any) {
    toast.add({
      severity: 'error',
      summary: 'Error',
...
// Clear dimension import results
const clearDimensionResults = () => {
  dimensionImportExport.value = new ItemImportResponse();
};
...
<!-- Dimension Import Section -->
<div class="bg-white rounded-lg p-6 shadow-md mt-6">
  <h2 class="text-xl font-semibold mb-2">Import Dimensions CSV data</h2>
  <p class="text-gray-600 mb-4">Upload a CSV file to start importing dimensions data</p>
  <div class="flex flex-col gap-4">
    <FileUpload mode="basic" name="importDimensionFile" accept=".csv" :maxFileSize="8000000" @select="handleDimensionFileSelect"
      :auto="false" chooseLabel="Select CSV File" />
    <div v-if="selectedFileDimensions" class="flex flex-col gap-4">
      <div class="flex items-center gap-2 p-3 bg-gray-50 rounded">
        <i class="pi pi-file text-gray-500"></i>
        <span class="text-gray-700">{{ selectedFileDimensions.name }}</span>
      </div>
      <div v-if="fb.loading['dimension.import']" class="w-full">
        <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
        <p class="text-sm text-gray-600 mt-2 text-center">Importing dimension data...</p>
      </div>
      <Button :label="fb.loading['dimension.import'] ? 'Importing...' : 'Import Dimensions'"
        severity="success"
        :icon="fb.loading['dimension.import'] ? 'pi pi-spinner pi-spin' : 'pi pi-save'"
        @click="handleDimensionSave"
        :loading="fb.loading['dimension.import']"
        :disabled="fb.loading['dimension.import']"
        class="p-button-md" />
    </div>
  </div>

  <!-- Dimension Import Results Section -->
  <div v-if="dimensionImportExport.invalid_records > 0 ||
              dimensionImportExport.updated_records > 0 ||
              dimensionImportExport.valid_records > 0"
       class="bg-white rounded-lg p-6 shadow-md mt-6">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-semibold">Dimensions Import Results</h2>
      <Button icon="pi pi-times" label="Clear Results" severity="secondary" outlined @click="clearDimensionResults" />
    </div>

    <!-- Summary Statistics -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
        <div class="text-2xl font-bold text-blue-600">{{ dimensionImportExport.total_records || 0 }}</div>
        <div class="text-sm text-blue-800">Total Records</div>
      </div>
      <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
        <div class="text-2xl font-bold text-orange-600">{{ dimensionImportExport.invalid_records || 0 }}</div>
        <div class="text-sm text-orange-800">Invalid Records</div>
      </div>
      <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
        <div class="text-2xl font-bold text-orange-600">{{ dimensionImportExport.duplicated_records || 0 }}</div>
        <div class="text-sm text-orange-800">Duplicate Records</div>
      </div>
      <div class="bg-purple-50 p-4 rounded-lg border border-purple-200">
        <div class="text-2xl font-bold text-purple-600">{{ dimensionImportExport.valid_records || 0 }}</div>
        <div class="text-sm text-purple-800">Records Inserted</div>
      </div>
    </div>

    <!-- Invalid Records DataTable -->
    <div v-if="dimensionImportExport.invalid_records > 0 && dimensionImportExport.invalid_data" class="mb-6">
      <h3 class="text-lg font-semibold mb-4 text-red-700">Invalid Records ({{ dimensionImportExport.invalid_records }})</h3>
      <DataTable v-model:filters="filters" :value="dimensionImportExport.invalid_data" paginator :rows="100" dataKey="row"
        filterDisplay="menu" :globalFilterFields="['data.dimension_code', 'data.dimension_name']" class="th-table">
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search invalid dimension records..." />
          </div>
        </template>
        <template #empty>No invalid dimension records found.</template>
        <Column field="row" header="Row" style="min-width: 6rem" />
        <Column field="errors" header="Errors" style="min-width: 15rem">
          <template #body="{ data }">
        ...
        <Column field="data.item_code" header="Item Code" style="min-width: 15rem" />
        <Column field="data.display_width" header="Display Width" style="min-width: 15rem" />
        <Column field="data.display_height" header="Display Height" style="min-width: 15rem" />
        <Column field="data.display_depth" header="Display Depth" style="min-width: 15rem" />
      </DataTable>
    </div>

    <!-- Duplicate Records DataTable -->
    <div v-if="dimensionImportExport.duplicated_records > 0 && dimensionImportExport.duplicated_data" class="mb-6">
      <h3 class="text-lg font-semibold mb-4 text-orange-700">Duplicate Records ({{ dimensionImportExport.duplicated_records }})</h3>
      <DataTable v-model:filters="filters" :value="dimensionImportExport.duplicated_data" paginator :rows="100" dataKey="dimension_id"
        filterDisplay="menu" :globalFilterFields="['item_code', 'display_depth', 'display_height', 'display_width']" class="th-table">
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search duplicate dimension records..." />
          </div>
        </template>
        <template #empty>No duplicate dimension records found.</template>
        <Column field="item_code" header="Item Code" style="min-width: 15rem" />
        <Column field="display_depth" header="Display Depth" style="min-width: 15rem" />
        <Column field="display_height" header="Display Height" style="min-width: 15rem" />
        <Column field="display_width" header="Display Width" style="min-width: 15rem" />
      </DataTable>
    </div>

    <!-- Updated Records DataTable -->
    <div v-if="dimensionImportExport.updated_records > 0 && dimensionImportExport.updated_data" class="mb-6">
      <h3 class="text-lg font-semibold mb-4 text-orange-700">Updated Records ({{ dimensionImportExport.updated_records }})</h3>
      <DataTable v-model:filters="filters" :value="dimensionImportExport.updated_data" paginator :rows="100" dataKey="dimension_id"
        filterDisplay="menu" :globalFilterFields="['item_code', 'display_depth', 'display_height', 'display_width']" class="th-table">
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search updated dimension records..." />
          </div>
        </template>
        <template #empty>No updated dimension records found.</template>
        <Column field="item_code" header="Item Code" style="min-width: 15rem" />
        <Column field="display_depth" header="Display Depth" style="min-width: 15rem" />
        <Column field="display_height" header="Display Height" style="min-width: 15rem" />
        <Column field="display_width" header="Display Width" style="min-width: 15rem" />
    ...
    <!-- Valid Records DataTable -->
    <div v-if="dimensionImportExport.valid_records > 0 && dimensionImportExport.valid_data">
      <h3 class="text-lg font-semibold mb-4 text-green-700">Inserted Records ({{ dimensionImportExport.valid_records }})</h3>
      <DataTable v-model:filters="filters" :value="dimensionImportExport.valid_data" paginator :rows="100" dataKey="dimension_id"
        filterDisplay="menu" :globalFilterFields="['item_code', 'display_depth', 'display_height', 'display_width']" class="th-table">
        <template #header>
          <div class="flex justify-between items-center">
            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
            <InputText v-model="filters['global'].value" placeholder="Search inserted dimension records..." />
          </div>
        </template>
        <template #empty>No inserted dimension records found.</template>
        <Column field="item_code" header="Item Code" style="min-width: 15rem" />
          <Column field="display_depth" header="Display Depth" style="min-width: 15rem" />
          <Column field="display_height" header="Display Height" style="min-width: 15rem" />
          <Column field="display_width" header="Display Width" style="min-width: 15rem" />
