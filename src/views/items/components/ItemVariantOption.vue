...
import type ItemModel from '@/models/Item.ts';
import { useItemStore } from '@/stores/item';

const emit = defineEmits(['update:product', 'saved', 'update:images', 'add:images']);
defineProps<{
  item?: ItemModel | null;
}>();
...
const { fb, itemVariant, item } = storeToRefs(itemStore);
...
const variantItemId = ref<number | null>(null);
...
const searchVariants = async (event: any) => {
  if (item.value.product_id !== null && item.value.product_id !== undefined) {
    productVariantSuggestions.value = await itemStore.listItemVariants(event, item.value.product_id);
  } else {
...
const searchOptionGroups = async (event: any, variant: VariantItem) => {
  if (variant.product_id) {
    const data = await itemStore.listItemOptionGroups(event, variant.product_id);
...
const searchOptions = async (data: { event: any, productId: number }) => {
  ...
  if (productId !== undefined && productId !== null) {
    optionSuggestions.value = await itemStore.listProductOptions(event, productId);
  } else {
    optionSuggestions.value = [];
  }
}
const searchTypes = async (query: string) => {
  typeSuggestions.value = await itemStore.listTypes(query);
}
...
const handleSaveVariant = async (data: any) => {
  try {
    data.item_id = route.params.id as string;
...
const handleUpdateVariant = async (data: any) => {
  try {
   data.variant_item_id = variantItemId.value ?? 0;
   data.item_id = route.params.id as string;
   console.log("this is data on handleUpdateVariant", data);
   
   const response = await itemStore.updateItemVariant(data);
   if (response) {
    fb.value.showToast({success: 'Variant updated successfully'}, toast);
   } else {
    fb.value.showToast({error: 'An error occurred while updating the variant.'}, toast);
   }
  } catch (error) {
...
const handleAddOptionGroup = async (data: any) => {
  ...
  const existingGroup = itemVariant.value?.itemOptionGroups.find(
    ...
    (g: ProductOptionGroup) => g.option_group_name === optionGroup.value.option_group_name
  );
  ...
  if (!data.index) {
    ...
    optionGroup.value.product_variant_id = itemVariant.value.product_variant_id ?? 0;
  }
  optionGroup.value.active = true;
...
const inputOptionGroup = async (data: any) => {
  // optionGroup.value = new ItemOptionGroup(data.value);
  optionGroup.value.isNew = true;
  optionGroup.value.product_option_group_id = 0 // "_new_" + (itemVariant.value?.itemOptionGroups.length);
  // optionGroup.value.item_option_group_id = data.value.product_option_group_id;
  optionGroup.value.active = true;
}
const inputOption = async (data: any) => {
  // const option = new ItemOption(data.value);
  // itemVariant.value.itemOptionGroups[0].itemOptions[0].product_option_group_id = 0
  // itemVariant.value.itemOptionGroups[0].itemOptions[0].product_option_id = 0


  option.value[0].product_option_group_id = 0;
  option.value[0].product_option_id = 0;
  // option.value.active = true;
  // option.value.item_option_id = 0;
}

const handleInsertOptionGroup = async (group: ItemOptionGroup) => {
  group.product_variant_id = itemVariant.value.product_variant_id ?? 0;
  console.log("this is group on handleInsertOptionGroup", group);
  itemVariant.value.itemOptionGroups.push(group);
  // itemVariant.value.product_id = productId as any;

  // await itemStore.createItemVariant(itemVariant.value);
...
const handleDeleteOptionGroup = async (data: any, variant: VariantItem) => {
  ...
  try {
    ...
    await itemStore.deleteItemOptionGroup(group);
...
const handleDeleteOption = async (data: any, variant: VariantItem) => {
  ...
  if (groupIndex !== undefined && groupIndex !== null && groupIndex > -1) {
    if (variant) {
      ...
      try {
        await itemStore.deleteItemOption(data.item_option_id, groupIndex);
...
onMounted(async () => {
  ...
  await itemStore.getItemVariantByItemId(route.params.id as string);
  productId.value = itemVariant.value.product_id;  
  variantItemId.value = itemVariant.value.variant_item_id;
});


</script>
<template>
  <Message v-if="fb.errors?.['item.variant.update.' + itemVariant?.product_variant_id]?.global_message
    || fb.errors?.['item.variant.create']?.global_message"
    class="br_content border-l-4 mb-5 w-full rounded-md shadow-sm">
    <span
      v-for="error in fb.errors?.['item.variant.update.' + itemVariant?.product_variant_id]?.global_message || fb.errors?.['item.variant.create']?.global_message"
  ...
  <div class="flex flex-col md:flex-row gap-2">
    <div class="loading-overlay border-2" v-if="fb.loading['item.edit.'+route.params.id]">
      <div class="flex justify-center items-center h-full spinner">
        ...
        <p class="mt-2">Loading item variant options data. Please wait.</p>
      </div>
    </div>
    <!-- Left Column -->
    <div
      ...
      <div class="border-2 p-3">
        <div class="flex items-center justify-between w-full">
          ...
          <Button v-if="!itemVariant.product_variant_id" label="Add" :icon="showVariantCard ? 'pi pi-minus' : 'pi pi-plus'"
            severity="secondary" rounded @click="toggleVariantForm" />
        </div>
      </div>

      <VariantForm v-if="!itemVariant?.product_variant_id"
        :fb="fb" v-show="showVariantCard" 
        ...
        @submit="handleSaveVariant" 
        @inputVariant="inputVariant"/>

      <Variant 
        :key="itemVariant.product_variant_id" 
        :variant="itemVariant"
        :showForm="!!itemVariant.selected" 
        :fb="fb" 
        :buttonLabel="'Update Variant'" 
        @update:variant="handleUpdateVariant"
        @delete:variant="handleDeleteVariant" 
        @edit:variant="toggleVariant()" 
        @inputVariant="inputVariant"
        @searchVariant="searchVariants"
        @add:variant="handleAddVariant"
      />
    ...
    <div ref="optionGroupsHeadingRef"
      ...
      <OptionGroupForm 
        ...
        :optionGroupData="itemVariant?.itemOptionGroups" 
        ...
        @searchOptionGroups="searchOptionGroups($event, itemVariant)"
        ...
        @delete:option="handleDeleteOption($event, itemVariant)" 
        @searchTypes="searchTypes($event)"
        @add:type="onSelectType($event)" 
        @inputOption="inputOption"
        @optionGroupInput="inputOptionGroup" 
      />

      <OptionGroupList 
        :variant="variantForList" 
        :fb="fb" 
        ...
        @update:optionGroup="handleUpdateOptionGroup"
        @update:variant="handleUpdateVariant" 
        @delete:optionGroup="handleDeleteOptionGroup($event, itemVariant)"
        @searchOptionGroups="searchOptionGroups($event, itemVariant)" 
        @searchOptions="searchOptions($event)"
        @add:option="handleAddOption" 
        @insert:option="handleInsertOption" 
