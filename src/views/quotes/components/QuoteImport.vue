...
const { importExport, fb } = storeToRefs(quoteStore);
// console.log('import page ', importExport);

const toast = useToast();
// quote Import related states
const selectedFileQuotes = ref<File | null>(null);
const filters = ref({ global: { value: null, matchMode: 'contains' } })
const showQuoteResults = computed(() => {
  return (
...
const handleFileSelect = (event: any) => {
  ...
  selectedFileQuotes.value = file;
};

// Handle variant import (save action)
const handleSave = async () => {
  if (!selectedFileQuotes.value) {
    toast.add({
      severity: 'error',
      summary: 'Error',
  ...
  try {
    ...
    formData.append('csv_file', selectedFileQuotes.value);
    await quoteStore.importQuotes(formData);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Quote imported successfully',
      life: 3000
    });
    selectedFileQuotes.value = null;
  } catch (error: any) {
...
const clearQuoteResults = () => {
  importExport.value = new QuoteImportResponse();
};
...
<template>
  <div class="variant-import p-6">
    <!-- Quotes Import Section -->
    <div class="bg-white rounded-lg p-6 shadow-md">
      <h2 class="text-xl font-semibold mb-2">Import Quotes CSV data</h2>
      <p class="text-gray-600 mb-4">Upload a CSV file to start importing quote data</p>
      <div class="flex flex-col gap-4">
        <FileUpload mode="basic" name="importQuoteFile" accept=".csv" :maxFileSize="1000000" @select="handleFileSelect"
          :auto="false" chooseLabel="Select CSV File" />
        <div v-if="selectedFileQuotes" class="flex flex-col gap-4">
          <div class="flex items-center gap-2 p-3 bg-gray-50 rounded">
            <i class="pi pi-file text-gray-500"></i>
            <span class="text-gray-700">{{ selectedFileQuotes.name }}</span>
          </div>
          <div v-if="fb.loading['quote.import']" class="w-full">
            <ProgressBar mode="indeterminate" style="height: 8px" class="w-full" />
            <p class="text-sm text-gray-600 mt-2 text-center">Importing quote data...</p>
          </div>
          <Button icon="pi pi-save" :label="fb.loading['quote.import'] ? 'Saving...' : 'Import quote'"
            severity="success" @click="handleSave" :loading="fb.loading['quote.import']"
            :disabled="fb.loading['quote.import']" class="p-button-md" />
        </div>
      </div>

      <!-- quotes Import Results Section -->
      <div v-if="showQuoteResults" class="bg-white rounded-lg p-6 shadow-md mt-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-semibold">quotes Import Results</h2>
          <Button icon="pi pi-times" label="Clear Results" severity="secondary" outlined
            @click="clearQuoteResults" />
        </div>
          ...
          <DataTable v-model:filters="filters" :value="importExport.invalid_data" paginator :rows="100" dataKey="row"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                ...
                <InputText v-model="filters['global'].value" placeholder="Search invalid quote records..." />
              </div>
            </template>
            <template #empty>No invalid quote records found.</template>
        ...
        <div v-if="importExport.duplicated_records > 0 && importExport.duplicated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.duplicated_data" paginator :rows="200" dataKey="row"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search duplicate quotes records..." />
              </div>
            </template>
            <template #empty>No invalid quotes records found.</template>
            <Column field="row" header="Row" style="min-width: 6rem">
        ...
        <div v-if="importExport.updated_records > 0 && importExport.updated_data" class="mb-6">
          ...
          <DataTable v-model:filters="filters" :value="importExport.updated_data" paginator :rows="100"
            ...
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search updated quote records..." />
              </div>
            </template>
            <template #empty>No updated quote records found.</template>
            <!-- <Column field="data.quote_type_id" header="Id" style="min-width: 10rem">
              <template #body="{ data }">{{ data.quote_type_id }}</template>
            </Column> -->
            <Column field="data.name" header="Name" style="min-width: 10rem">
        ...
        <div v-if="importExport.valid_records > 0 && importExport.valid_data">
          ...
          <DataTable v-model:filters="filters" :value="importExport.valid_data" paginator :rows="100" dataKey="quote_type_id"
            filterDisplay="menu" :globalFilterFields="['name', 'unit', 'value']" class="th-table">
            <template #header>
              <div class="flex justify-between items-center">
                <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter()" />
                <InputText v-model="filters['global'].value" placeholder="Search valid quote records..." />
              </div>
            </template>
