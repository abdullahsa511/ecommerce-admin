...
const refreshTabs = () => {
  ...
  if (routeId) {
    ...
    const itemRoute = `/ecommerce/sales/quotes/item/${routeId}`;
    const existItemRoute = items.value.find(item => item.route === itemRoute);
    if (!existItemRoute) {
      items.value.push({ route: itemRoute, label: 'Item', icon: 'pi pi-list' });
    }
  } else {
    items.value = items.value.filter(item => !item.route.includes('/edit'));
    items.value = items.value.filter(item => !item.route.includes('/item'));
  }
}
// refreshTabs();
