...
const emit = defineEmits(['update:language', 'saved', 'editTabUnmunted', 'editTabMounted']);
...
onMounted(async () => {
    emit('editTabMounted');
    await languageStore.fetchLanguageById(languageId);
});
...
<template>
    <div v-if="fb.loading['language.edit.' + languageId]" class="absolute inset-0 bg-white bg-opacity-75 flex items-center justify-center z-10">
        <div class="text-center">
            <i class="pi pi-spin pi-spinner" style="font-size: 2rem;"></i>
            <p class="mt-2">Loading language details...</p>
        </div>
    </div>
    <Fluid class="flex flex-col md:flex-row gap-8 mb-20">
        <div class="w-full">
            <div class="flex flex-col gap-6">
                <h3 class="text-lg font-medium border-b border-gray-200 pb-2">Edit Language</h3>
                ...
                <div class="grid grid-cols-12 gap-2">
                    ...
                    <div class="col-span-12 md:col-span-10">
                        <InputText class="text-lg w-full" id="name" type="text" v-model="newLanguage.name" placeholder="Name"
                         :class="{'p-invalid': fb.errors['language.update.' + languageId]?.name}"
                        />
                        <Message v-if="fb.errors['language.update.' + languageId]?.name" severity="error" size="small" variant="title">
                            <span v-for="error in fb.errors['language.update.' + languageId]?.name" :key="error" class="flex flex-col">
                                {{ error }}
                            </span>
                        </Message>
                    </div>
                ...
                <div class="grid grid-cols-12 gap-2">
                    ...
                    <div class="col-span-12 md:col-span-10">
                        <InputText class="text-lg w-full" id="code" type="text" v-model="newLanguage.code" placeholder="Code"
                         :class="{'p-invalid': fb.errors['language.update.' + languageId]?.code}" :disabled="true"
                        />
                        <Message v-if="fb.errors['language.update.' + languageId]?.code" severity="error" size="small" variant="title">
                            <span v-for="error in fb.errors['language.update.' + languageId]?.code" :key="error" class="flex flex-col">
                                {{ error }}
                            </span>
                        </Message>
                    </div>
                ...
                <div class="grid grid-cols-12 gap-2">
                    ...
                    <div class="col-span-12 md:col-span-10">
                        <InputText class="text-lg w-full" id="locale" type="text" v-model="newLanguage.locale" placeholder="Locale"
                         :class="{'p-invalid': fb.errors['language.update.' + languageId]?.locale}"
                        />
                        <Message v-if="fb.errors['language.update.' + languageId]?.locale" severity="error" size="small" variant="title">
                            <span v-for="error in fb.errors['language.update.' + languageId]?.locale" :key="error" class="flex flex-col">
                                {{ error }}
                            </span>
                        </Message>
                    </div>
