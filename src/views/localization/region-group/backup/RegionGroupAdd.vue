...
const saveRegionGroup = async () => {
    try {
        await regionGroupStore.createRegionGroup(newRegionGroup.value);
        toast.add({
            severity: 'success',
            summary: 'Success',
            detail: 'Region group created successfully',
            life: 3000
        });
        goToRegionGroupList();
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to create region group. Please try again.',
            life: 3000
        });
    }
};
