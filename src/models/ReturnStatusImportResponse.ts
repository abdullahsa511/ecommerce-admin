...
interface ImportSummary {
    success_rate: string;
    return_status_processed: number;
    [key: string]: any; // For additional summary fields
}

interface IReturnStatusImportResponse {
    return_statuses: {
        inserted_count: number;
        valid_data: ReturnStatusData[];
    };
    duplicated_data: any[];
    ...
    updated_data: ReturnStatusData[];
    updated_records: number;
    valid_records: number;
    valid_data: ReturnStatusData[];
}
export default class ReturnStatusImportResponse implements IReturnStatusImportResponse {
    return_statuses: {
        inserted_count: number;
        valid_data: ReturnStatusData[];
    };
    ...
    valid_records: number;
    valid_data: ReturnStatusData[];
    constructor(data: Partial<IReturnStatusImportResponse> = {}) {
        this.return_statuses = data.return_statuses ?? { inserted_count: 0, valid_data: [] };
        ...
        this.summary = data.summary ?? { success_rate: '', return_status_processed: 0 };
