...
interface ImportSummary {
    success_rate: string;
    stock_status_processed: number;
    [key: string]: any; // For additional summary fields
}

interface IStockStatusImportResponse {
    stock_statuses: {
        inserted_count: number;
        valid_data: StockStatusData[];
    };
    duplicated_data: any[];
    ...
    updated_data: StockStatusData[];
    updated_records: number;
    valid_records: number;
    valid_data: StockStatusData[];
}
export default class StockStatusImportResponse implements IStockStatusImportResponse {
    stock_statuses: {
        inserted_count: number;
        valid_data: StockStatusData[];
    };
    ...
    valid_records: number;
    valid_data: StockStatusData[];
    constructor(data: Partial<IStockStatusImportResponse> = {}) {
        this.stock_statuses = data.stock_statuses ?? { inserted_count: 0, valid_data: [] };
        ...
        this.summary = data.summary ?? { success_rate: '', stock_status_processed: 0 };
