interface RoleData {
    name: string;
    display_name: string;
    permissions: string;
}

interface ImportSummary {
    success_rate: string;
    role_processed: number;
    permissions_processed: number;
    [key: string]: any; // For additional summary fields
}

interface IRoleImportResponse {
    roles: {
        inserted_count: number;
        valid_data: RoleData[];
    };
    duplicated_data: any[];
    duplicated_records: number;
    invalid_data: any[];
    invalid_records: number;
    success: boolean;
    summary: ImportSummary;
    total_records: number;
    updated_data: RoleData[];
    updated_records: number;
    valid_records: number;
    valid_data: RoleData[];
}
export default class RoleImportResponse implements IRoleImportResponse {
    roles: {
        inserted_count: number;
        valid_data: RoleData[];
    };
    duplicated_data: any[];
    duplicated_records: number;
    invalid_data: any[];
    invalid_records: number;
    success: boolean;
    summary: ImportSummary;
    total_records: number;
    updated_data: RoleData[];
    updated_records: number;
    valid_records: number;
    valid_data: RoleData[];
    constructor(data: Partial<IRoleImportResponse> = {}) {
        this.roles = data.roles ?? { inserted_count: 0, valid_data: [] };
        this.duplicated_data = data.duplicated_data ?? [];
        this.duplicated_records = data.duplicated_records ?? 0;
        this.invalid_data = data.invalid_data ?? [];
        this.invalid_records = data.invalid_records ?? 0;
        this.success = data.success ?? false;
        this.summary = data.summary ?? { success_rate: '', role_processed: 0, permissions_processed: 0 };
        this.total_records = data.total_records ?? 0;
        this.updated_data = data.updated_data ?? [];
        this.updated_records = data.updated_records ?? 0;
        this.valid_records = data.valid_records ?? 0;
        this.valid_data = data.valid_data ?? [];
    }
}