...
interface ImportSummary {
    success_rate: string;
    tax_processed: number;
    [key: string]: any; // For additional summary fields
}

interface ITaxRateImportResponse {
    tax_rates: {
        inserted_count: number;
        valid_data: TaxRateData[];
    };
    duplicated_data: any[];
    ...
    updated_data: TaxRateData[];
    updated_records: number;
    valid_records: number;
    valid_data: TaxRateData[];
}
export default class TaxRateImportResponse implements ITaxRateImportResponse {
    tax_rates: {
        inserted_count: number;
        valid_data: TaxRateData[];
    };
    ...
    valid_records: number;
    valid_data: TaxRateData[];
    constructor(data: Partial<ITaxRateImportResponse> = {}) {
        this.tax_rates = data.tax_rates ?? { inserted_count: 0, valid_data: [] };
        ...
        this.summary = data.summary ?? { success_rate: '', tax_processed: 0 };
