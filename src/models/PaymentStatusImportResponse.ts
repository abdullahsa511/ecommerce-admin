...
interface ImportSummary {
    success_rate: string;
    payment_status_processed: number;
    [key: string]: any; // For additional summary fields
}

interface IPaymentStatusImportResponse {
    payment_statuses: {
        inserted_count: number;
        valid_data: PaymentStatusData[];
    };
    duplicated_data: any[];
    ...
    updated_data: PaymentStatusData[];
    updated_records: number;
    valid_records: number;
    valid_data: PaymentStatusData[];
}
export default class PaymentStatusImportResponse implements IPaymentStatusImportResponse {
    payment_statuses: {
        inserted_count: number;
        valid_data: PaymentStatusData[];
    };
    ...
    valid_records: number;
    valid_data: PaymentStatusData[];
    constructor(data: Partial<IPaymentStatusImportResponse> = {}) {
        this.payment_statuses = data.payment_statuses ?? { inserted_count: 0, valid_data: [] };
        ...
        this.summary = data.summary ?? { success_rate: '', payment_status_processed: 0 };
