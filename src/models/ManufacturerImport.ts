...
interface ImportSummary {
    success_rate: string;
    manufacturer_processed: number;
    [key: string]: any; // For additional summary fields
}

interface IManufacturerImportResponse {
    manufacturer_contents: {
        inserted_count: number;
        valid_data: ManufacturerData[];
    };
    manufacturers: {
        inserted_count: number;
        valid_data: ManufacturerData[];
    ...
    updated_data: ManufacturerData[];
    updated_records: number;
    valid_records: number;
    valid_data: ManufacturerData[];
}
export default class ManufacturerImportResponse implements IManufacturerImportResponse {
    manufacturer_contents: {
        inserted_count: number;
        valid_data: ManufacturerData[];
    };
    ...
    valid_records: number;
    valid_data: ManufacturerData[];
    constructor(data: Partial<IManufacturerImportResponse> = {}) {
        this.manufacturer_contents = data.manufacturer_contents ?? { inserted_count: 0, valid_data: [] };
        this.manufacturers = data.manufacturers ?? { inserted_count: 0, valid_data: [] };
        ...
        this.summary = data.summary ?? { success_rate: '', manufacturer_processed: 0 };
