...
export const useProductOptionStore = defineStore('productOption', () => {
    ...
    const getProductOptions = async () => {
        // Use console.log instead of alert to avoid runtime errors in non-browser environments.
        console.log('fetchProductOptions');
        ...
        try {
            ...
            // ProductOptionService returns the response data (array)
            productOptions.value = Array.isArray(response)
                ? response.map(opt => new ProductOption(opt))
                : (response.data || []).map(opt => new ProductOption(opt));
